...
SELECT_FREQ_TO_RRULE = {'daily': rrule.DAILY, 'weekly': rrule.WEEKLY,
    'monthly': rrule.MONTHLY, 'yearly': rrule.YEARLY, }

RRULE_FREQ_TO_SELECT = {rrule.DAILY: 'daily', rrule.WEEKLY: 'weekly',
    rrule.MONTHLY: 'monthly', rrule.YEARLY: 'yearly', }

RRULE_WEEKDAY_TO_FIELD = {rrule.MO.weekday: 'mon', rrule.TU.weekday: 'tue',
    rrule.WE.weekday: 'wed', rrule.TH.weekday: 'thu', rrule.FR.weekday: 'fri',
    rrule.SA.weekday: 'sat', rrule.SU.weekday: 'sun', }

RRULE_WEEKDAYS = {'SUN': 'SU', 'MON': 'MO', 'TUE': 'TU', 'WED': 'WE',
                  'THU': 'TH', 'FRI': 'FR', 'SAT': 'SA'}

RRULE_TYPE_SELECTION = [('daily', 'Días'), ('weekly', 'Semanas'),
    ('monthly', 'Meses'), ('yearly', 'Años'), ]

END_TYPE_SELECTION = [('count', 'Número de repeticiones'),
    ('end_date', 'Fecha final'), ('forever', 'Para siempre'), ]

MONTH_BY_SELECTION = [('date', 'Fecha del mes'), ('day', 'Día del mes'), ]

WEEKDAY_SELECTION = [('MON', 'Lunes'), ('TUE', 'Martes'), ('WED', 'Miércoles'),
    ('THU', 'Jueves'), ('FRI', 'Viernes'), ('SAT', 'Sábado'),
    ('SUN', 'Domingo'), ]

BYDAY_SELECTION = [('1', 'Primero'), ('2', 'Segundo'), ('3', 'Tercero'),
    ('4', 'Cuarto'), ('-1', 'Ultimo'), ]
...
class RecurrenceRule(models.Model):
    ...
    def _compute_name(self):
        for recurrence in self:
            ...
            every = "Cada %(count)s %(period)s" % {
                'count': recurrence.interval, 'period': period}

            if recurrence.end_type == 'count':
                end = "por %s eventos" % (recurrence.count or 0)
            elif recurrence.end_type == 'end_date':
                end = "hasta el %s" % (recurrence.until or '')
            else:
                end = ''
            ...
            if recurrence.rrule_type == 'weekly':
                ...
                day_strings = [d[1] for d in WEEKDAY_SELECTION if
                               d[0] in weekday_short]
                on = "en %s" % ", ".join(
                    [day_name for day_name in day_strings])
            elif recurrence.rrule_type == 'monthly':
                if recurrence.month_by == 'day':
                    weekday_label = dict(BYDAY_SELECTION)[recurrence.byday]
                    on = "sobre el %(position)s %(weekday)s" % {
                        'position': recurrence.byday, 'weekday': weekday_label}
                else:
                    on = "día %s" % (recurrence.day or '')
            else:
                on = ''
