...
class CalendarPlanReport(models.Model):
    ...
    type = fields.Selection([('all', 'Todos los eventos'), ('selected', 'Organizados por mí')], default='all',
                            required=True, string="Tipo")
    plan = fields.Many2one('calendar.tipo.calendario', string='Plantilla Impresión', required=True)
    ...
    def report_busca_dias(self):
        ...
        while dia < dias_final:
            data = {'dia': dia,
                    'nombre': date(anio, mes, dia).strftime('%A').capitalize(),
                    }
    ...
    def report_busca_calendario(self):
        ...
        if self.type == 'all':
            ...
            records = self.env['calendar.event'].search([('partner_ids', 'in', partner)])
                ...
                if obj.start.month == self.codigo_mes and obj.start.year == int(self.anio):
                    if obj.allday:
                        ...
                        hora_control = datetime.strftime(
                            obj.start.replace(tzinfo=UTC).astimezone(timezone(tz)).replace(tzinfo=None), "%H:%M")
                    ...
                    data = {'evento': obj.name,
                            ...
                            obj.start.replace(tzinfo=UTC).astimezone(timezone(tz)).replace(tzinfo=None), "%d-%m-%Y"),
                            'hora': hora_control,
                            'dia': obj.start.day,
                            'mes': obj.start.month,
                            'anio': obj.start.year,
            ...
            records = self.env['calendar.event'].search([('user_id', '=', usuario)])
    ...
    def generar_reporte(self):
        if self.plan_dvpe:
            return self.env.ref('sicpro_app_calendario.calendar_plan_trabajo_dvpe_report_action').report_action([], )
        else:
            return self.env.ref('sicpro_app_calendario.calendar_plan_trabajo_report_action').report_action([], )
