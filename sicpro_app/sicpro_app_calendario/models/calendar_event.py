...
class Meeting(models.Model):
    ...
    @api.model
    def _default_tipo(self):
        data = self.env['calendar.tipo.calendario'].search([('tipo_defecto', '=', True), ])
        return data

    start = fields.Datetime('Comienza el', required=True, tracking=True, default=fields.Datetime.now,
                            help="Fecha de inicio del evento, sin hora para eventos que duran el día completo")
    stop = fields.Datetime('Termina a las', required=True, tracking=True, readonly=False,
                           default=lambda self: fields.Datetime.now() + timedelta(hours=1), compute='_compute_stop',
                           store=True, help="Fecha de fin del evento, sin hora para eventos que duran el día completo")

    tipo = fields.Many2one('calendar.tipo.calendario', string='Tipo de Actividad', default=_default_tipo,
                           required=False)
    prioridad = fields.Selection(string='Prioridad', related='tipo.prioridad', store=True)
    participantes_ids = fields.Many2many('sicpro.app.trabajadores.cargos', 'calendarios_cargos_trabajadores_rel',
                                         string='Participantes', required=False)
    tipo_dvpe = fields.Boolean(string='Inf. DVPE', related='tipo.tipo_dvpe')
    ubicacion = fields.Many2one(comodel_name='sicpro.app.reuniones.lugares', string='Ubicación', required=False)
    actividades_organizativas = fields.Many2one(comodel_name='calendar.actividades.organizativas',
                                                string='Actividades Organizativas', required=False)
    tipo_ubicacion = fields.Selection(string='Tipo Ubicación',
                                      selection=[('otro', 'Otra'), ('interno', 'Interna'), ('externo', 'Externa'), ],
                                      required=True, default='otro')
    dirige = fields.Many2many('res.users', 'calendar_dirige_res_users_rel', string='Dirige')
    dirige_externos = fields.Many2many('calendar.cargos.externos', 'calendar_dirige_cargos_externos_rel',
    ...
    cumplimiento_tarea = fields.Selection(
        string='Cumplimiento', selection=[('cumplida', 'Tarea Cumplida'), ('incumplida', 'Tarea Incumplida'), ],
        required=False, )
    incumplimiento_causa = fields.Text(string="Causa del incumplimiento", required=False)
    modificacion_tarea = fields.Boolean(string='Tarea modificada', default=False, )
    modificacion_usuario = fields.Many2many('res.users', 'calendar_modificar_tarea_res_users_rel', string='Modificaron')
    tipo_tarea = fields.Selection(
        string='Tipo de Tarea', required=False,
        selection=[('principales', 'Tareas Principales'), ('generales', 'Tareas Generales'), ],)
    ...
    def create(self, vals_list):
        ...
        vals_list = [  # Else bug with quick_create when we are filter on an other user
            dict(vals, user_id=self.env.user.id) if not 'user_id' in vals else vals for vals in vals_list]
        ...
        events.filtered(lambda event: event.start > fields.Datetime.now()).attendee_ids._send_mail_to_attendees(
            self.env.ref('sicpro_app_calendario.sicpro_calendar_template_meeting_invitation', raise_if_not_found=False))
    ...
    def write(self, values):
        ...
        if 'partner_ids' in values:
            ...
            (current_attendees - previous_attendees)._send_mail_to_attendees(
                self.env.ref('sicpro_app_calendario.sicpro_calendar_template_meeting_invitation',
        ...
        if not self.env.context.get('is_calendar_event_new') and 'start' in values:
            ...
            if start_date and start_date >= fields.Datetime.now():
                (current_attendees & previous_attendees).with_context(
                    ...
                    self.env.ref('sicpro_app_calendario.sicpro_calendar_template_meeting_changedate',
                                 raise_if_not_found=False))
                # envío el correo electrónico
                for participantes in self.attendee_ids:
                    email_values = {'email_to': participantes.partner_id.email_formatted, }
                    local_context = self.env.context.copy()
                    template = self.env.ref('sicpro_app_calendario.sicpro_calendar_template_meeting_changedate')
    ...
    def action_open_composer(self):
        if not self.partner_ids:
            raise UserError(_("No existen participantes en el evento."))
        template_id = self.env['ir.model.data']._xmlid_to_res_id(
            'sicpro_app_calendario.sicpro_calendar_template_meeting_update', raise_if_not_found=False)
        ...
        compose_ctx = dict(default_composition_mode=composition_mode, default_model='calendar.event',
                           default_res_ids=self.ids, default_use_template=bool(template_id),
                           default_template_id=template_id, default_partner_ids=self.partner_ids.ids,
    ...
    def action_enviar_actualizacion_evento(self):
        ...
        if email:
            ...
            for participantes in self.partner_ids:
                ...
                local_context = self.env.context.copy()
                template = self.env.ref('sicpro_app_calendario.sicpro_calendar_template_meeting_update')
