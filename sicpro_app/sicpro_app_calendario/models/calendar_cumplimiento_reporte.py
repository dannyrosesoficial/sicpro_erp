...
class CalendarCumplimientoReport(models.Model):
    ...
    type = fields.Selection(
        [('calendario', 'Tipo de Calendario'), ('individual', 'Individuales')],
        default='calendario', required=True, string="Tipo")
    tipo_calendario = fields.Many2one('calendar.tipo.calendario',
                                      string='Tipo de Calendario',
                                      required=False)
    ...
    def compute_cuenta_tareas(self):
        ...
        if self.type == 'calendario':
            ...
            tareas_modificadas = self.env['calendar.event'].search(
                [('tipo', '=', self.tipo_calendario.name),
                 ('modificacion_tarea', '=', True)])
            for obj in tareas_modificadas:
                if obj.start.month == self.codigo_mes and obj.start.year == int(
                    self.anio):
                    t_modificadas += 1

            data = {'planificadas': t_planificadas, 'cumplidas': t_cumplidas,
                'incumplidas': t_incumplidas, 'modificadas': t_modificadas,
                'nuevas': t_nuevas, }
    ...
    def report_busca_actividades(self):
        ...
        if self.type == 'calendario':
            ...
            for obj in tareas_incumplidas:
                if obj.start.month == self.codigo_mes and obj.start.year == int(
                    self.anio):
                    data_incumplidas = {'evento': obj.name,
            ...
            for obj in tareas_modificadas:
                if obj.start.month == self.codigo_mes and obj.start.year == int(
                    self.anio):
                    data_modificadas = {'evento': obj.name,
    ...
    def generar_reporte(self):
        return self.env.ref(
            'sicpro_app_calendario.calendar_plan_trabajo_cumplimiento_report_action').report_action(
            [], )
