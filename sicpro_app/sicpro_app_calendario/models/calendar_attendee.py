...
class Attendee(models.Model):
    ...
    def _send_mail_to_attendees(self, mail_template, force_send=False):
        ...
        if self.env['ir.config_parameter'].sudo().get_param(
            'calendar.block_mail') or self.env.context.get(
            "no_mail_to_attendees"):
            return False
        if not mail_template:
            _logger.warning(
                "No template passed to %s notification process. Skipped.",
                self)
            return False
        ...
        for attendee in self:
            if attendee.email and attendee.partner_id != self.env.user.partner_id:
                ...
                attachment_values = []
                if ics_file:
                    attachment_values = [(0, 0, {'name': 'Invitación.ics',
                                                 'mimetype': 'text/calendar',
                                                 'datas': base64.b64encode(
                                                     ics_file)})]

                body = mail_template._render_field('body_html', attendee.ids,
                                                   compute_lang=True,
                                                   post_process=True)[
                    attendee.id]
                subject = mail_template._render_field('subject', attendee.ids,
                                                      compute_lang=True)[
                    attendee.id]

                attendee.event_id.with_context(
                    no_document=True).message_notify(
                    ...
                    attachment_ids=attachment_values, force_send=force_send)
