    ...
    def _send_mail_to_attendees(self, mail_template, force_send=False):
        ...
        for attendee in self:
            if attendee.email and attendee.partner_id != self.env.user.partner_id:
                ...
                if ics_file:
                    attachment_values = [
                        (0, 0, {'name': 'Invitación.ics',
                                'mimetype': 'text/calendar',
                                'datas': base64.b64encode(ics_file)})
                    ]

                body = mail_template._render_field('body_html', attendee.ids, compute_lang=True, post_process=True)[attendee.id]
                subject = mail_template._render_field('subject', attendee.ids, compute_lang=True)[attendee.id]
