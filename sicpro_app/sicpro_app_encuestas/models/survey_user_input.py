...
class SurveyUserInput(models.Model):
    """Entrada de usuario heredada para que sea aceptable para preguntas personalizadas"""
    _inherit = "survey.user_input"

    trabajador_id = fields.Many2one('sicpro.app.trabajadores', string='Trabajador', required=False)

    def save_lines(self, question, answer, comment=None):
        """Función para guardar respuesta personalizadas"""
        old_answers = self.env['survey.user_input.line'].search(
            [('user_input_id', '=', self.id), ('question_id', '=', question.id), ])
        if question.question_type in ['password', 'range', 'time', 'url', 'email', 'range', 'file',
                                      'many2one', 'week', 'color', 'month', 'address', 'name', 'selection', 'file',
                                      'qr', 'barcode']:
    ...
    def _save_line_matrix(self, question, old_answers, answers, comment):
        """Función para guardar matriz personalizada"""
        if question.matrix_subtype == 'custom':
            self._save_line(question, answers)
        else:
            vals_list = []
            if not answers and question.matrix_row_ids:
                # agregar una respuesta falsa para forzar a guardar una línea omitida
                # esto hará que esta pregunta se considere correctamente como
                # omitido en las estadísticas
    ...
    def _save_line_selection_answer(self, question, answer):
        """Función para guardar respuestas del tipo de selección."""
