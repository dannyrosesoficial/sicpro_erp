...
class AppClientes(models.Model):
    ...
    tipo_cliente = fields.Selection([('ldap', 'LDAP'), ('local', 'Local'), ],
                                    default='ldap', string='Tipo de Cliente')
    pep = fields.Char(string='No. Plaza', required=False)
    buscar_pep = fields.Many2one(
        comodel_name='sicpro.app.modulo.ldap.registros',
        string='Buscar No. PEP', required=False)
    ...
    def buscar_usuario_ldap(self):
        if self.buscar_pep:
            ...
            uo = self.env['sicpro.nomenclador.territorios'].sudo().search(
                [('abreviatura', '=', self.buscar_pep.ou)]).id
            territorio = self.env['sicpro.app.clientes'].sudo().search(
                ['&', ('tipo_registro', '=', 'entidad'),
                 ('territorio', '=', uo)]).id
            if territorio:
