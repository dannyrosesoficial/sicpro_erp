...
class IrCronHistory(models.Model):
    ...
    ir_actions_server_id = fields.Many2one('ir.actions.server', 'Acción del Servidor',
                                           delegate=True, ondelete='cascade', required=True)
    action_name = fields.Char(string='Cron', readonly=True)
    date_start = fields.Datetime(string='Inicio', default=lambda self: fields.Datetime.now())
    date_end = fields.Datetime(string='Fin', )
    state = fields.Selection([('in_progress', 'En Progreso'), ('done', 'Ejecutado'), ('error', 'Error'),
                              ('interrupted', 'Interrumpido'), ], 'Estado', default='in_progress', readonly=True)
    message_error = fields.Text(string='Mensaje de Error', )

    @api.model
    def _create_history(self, cron_name, server_action_id):
        self.search([('ir_actions_server_id', '=', server_action_id), ('state', '=', 'in_progress')]).write(
            {'state': 'interrupted'})
        # Creo historial
        cron_history = self.sudo().create({'ir_actions_server_id': server_action_id, 'action_name': cron_name})
        self._cr.commit()
        return cron_history

    def _done_history(self):
        if self.state == 'in_progress':
            self.sudo().write({'state': 'done', 'date_end': fields.Datetime.now(), })

    @api.model
    def _error_history(self, job_exception):
        cron_history_id = self._context.get('cron_history_id')
        if cron_history_id:
            cron_history = self.env['ir.cron.history'].browse(cron_history_id).sudo()
    ...
    def cron_cleanup_cron_history(self, days=90):
        ...
        _logger.info('Se limpio el historial: {}'.format(len_histories))
