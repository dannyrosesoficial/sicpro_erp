...
class ResUsers(models.Model):
    ...
    def _auth_timeout_get_ignored_urls(self):
        """Verífica las URL ignoradas en los valores predeterminados del parámetro de configuración almacenado"""
    ...
    def _auth_timeout_check(self):
        """Realice la validación del tiempo de espera de la sesión y la expira si es necesario."""
        if not http.request:
            return
        ...
        # Si la sesión terminó, todo listo
        if terminated:
            raise SessionExpiredException("La sesión del usuario ha expirado")

        # De lo contrario, actualice condicionalmente la sesión modificada y los tiempos de acceso
        ...
        if http.request.httprequest.path not in ignored_urls:
            ...
            try:
                ...
                _logger.exception(
                    "Excepción de actualización de acceso a archivos de sesión, tiempos modificados.",
                )
