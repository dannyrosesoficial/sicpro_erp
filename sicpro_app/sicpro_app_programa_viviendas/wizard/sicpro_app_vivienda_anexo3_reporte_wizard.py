...
class ViviendaAnexo3(models.TransientModel):
    ...
    especialista_obras = fields.Many2one('sicpro.app.trabajadores',
                                         string='Especialista en Obras de Ingeniería',
                                         required=False)
    jefe_logistica = fields.Many2one('sicpro.app.trabajadores',
                                     string='Jefe Dpto. Logística y Servicios',
                                     required=False)
    ...
    def report_busca_programa(self):
        ...
        if self.reporte_general:
            ...
            for item in materiales_ids:
                ...
                if cantidad != 0:
                    data = {'material': item.name, 'um': item.um.name,
                            'cantidad': cantidad, 'unitario': None,
                            'total': None, }
            ...
            if self.etapa:
                ...
                records = self.env[
                    'sicpro.app.vivienda.trabajador.productos'].sudo().search(
                    ['&', ('solicitud_id.etapa', '=', self.etapa.id),
                     ("estado", "in", ['aprobado', 'entregado'])])
    ...
    def generar_reporte(self):
        return self.env.ref(
            'sicpro_app_programa_viviendas.informe_modelo_vivienda_anexo_3_action').report_action(
            [], )
