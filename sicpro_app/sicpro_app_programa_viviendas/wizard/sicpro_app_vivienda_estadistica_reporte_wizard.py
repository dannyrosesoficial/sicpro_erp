...
class ViviendaAnexoEstadistica(models.TransientModel):
    ...
    etapa = fields.Many2one(comodel_name='sicpro.app.vivienda.etapas', string='Etapa', required=True)
    ...
    def report_busca_programa(self):
        ...
        if self.etapa:
            ...
            viviendas_ids = self.env['sicpro.app.vivienda.trabajador'].sudo().search(
                ['&', ('etapa', '=', self.etapa.id), ("estado", "in", ['logistica', 'terminado'])])
            # Busco los valores de gastos de la etapa
            etapas_id = self.env['sicpro.app.vivienda.etapas'].sudo().search([('id', '=', self.etapa.id)])
            # Busco todos los materiales
            materiales_ids = self.env['sicpro.app.vivienda.materiales'].sudo().search([('active', '=', True)])
            # Busco todos los materiales
            productos_ids = self.env['sicpro.app.vivienda.trabajador.productos'].sudo().search(
                ['&', ('etapa', '=', self.etapa.id), ("estado", "in", ['aprobado', 'entregado'])])
            ...
            for item in materiales_ids:
                ...
                if cantidad != 0:
                    data = {'material': item.name, 'um': item.um.name, 'cantidad': cantidad, }
                    materiales.append(data)

            data = {'beneficiados': beneficiados, 'mujeres': mujeres, 'asignado': etapas_id.monto,
                    'ejecutado': etapas_id.monto_usado, 'etapa': self.etapa.name,
                    'pendiente_facturar': etapas_id.pago_anticipado, 'avance': avance, 'materiales': materiales, }
    ...
    def generar_reporte(self):
        return self.env.ref('sicpro_app_programa_viviendas.informe_modelo_vivienda_estadisticas_action').report_action([], )
