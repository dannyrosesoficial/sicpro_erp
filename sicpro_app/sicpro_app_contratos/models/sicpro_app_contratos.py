...
class Contratos(models.Model):
    ...
    kanban_state = fields.Selection(
        [('normal', 'Borrador'), ('blocked', 'Rechazado'),
         ('done', 'Aprobado'), ], string='Estado interno', copy=False,
        default='normal', readonly=True)
    cuentas_ids = fields.One2many('sicpro.app.contratos.cuentas', 'name',
                                  'Cuentas')
    proveedor = fields.Many2one('sicpro.app.contratos.proveedores',
                                string='Proveedor', tracking=True, index=True,
                                required=True,
                                domain="[('stage_id.is_won', '=', True)]")
    tipo_proveedor = fields.Many2one(
        comodel_name='sicpro.app.contratos.proveedores.tipo',
        string='Tipo Proveedor', related='proveedor.tipo', store=True)
    ...
    presupuesto_cup = fields.Monetary('Valor del Contrato', tracking=True,
    ...
    area_contrato = fields.Many2one('sicpro.app.contratos.areas',
                                    required=True, string='Área del contrato',
    ...
    estado_interno = fields.Selection(
        [('nuevo', 'Nuevo'), ('liberado', 'Liberado'),
         ('contratacion', 'Contratación'), ('revisado', 'Revisado'),
         ('legal', 'Legal'), ('economia', 'Economía'),
         ('economia_dc', 'Economía DC'), ('firma_director', 'Firma Director'),
         ('firma_proveedor', 'Firma Proveedor'), ('activo', 'Activo'),
         ('devuelto', 'Devuelto'), ('cancelado', 'Cancelado'),
         ('terminado', 'Terminado')], index=True, required=True,
        default=lambda self: 'nuevo')
    sequence_consecutivo = fields.Char(string='Secuencia del contrato',
    ...
    fecha_comite_contratacion = fields.Date(string="Fecha del Comité",
                                            required=False, )
    contratacion_fecha_inicial = fields.Date(
        string="Fecha Inicial Contratación", required=False, )
    contratacion_persona_contratacion = fields.Many2one('res.users',
                                                        string='Emitido por:',
                                                        index=True,
                                                        tracking=True)
    contratacion_fecha_fin = fields.Date(string="Fecha Fin Contratación",
                                         required=False, )
    ...
    ##########################################################################
    solicitud_fecha_inicial = fields.Date(string="Fecha Inicial Solicitud",
                                          required=False,
                                          default=lambda self: fields.Date.context_today(self))
    solicitud_persona_solicita = fields.Many2one('res.users',
                                                 string='Solicitado por:',
                                                 default=lambda
                                                     self: self.env.uid,
                                                 index=True, tracking=True)
    solicitud_fecha_fin = fields.Date(string="Fecha Fin Solicitud",
                                      required=False, )
    ...
    ##########################################################################
    revisado_fecha_inicial = fields.Date(string="Revisado Fecha Inicial",
                                         required=False, )
    revisado_persona_revisa = fields.Many2one('res.users',
                                              string='Contrato revisado por:',
                                              index=True, tracking=True)
    revisado_detalles_contrato = fields.Html(string='Detalles Revisado')
    revisado_fecha_fin = fields.Date(string="Fecha Fin Revisado",
                                     required=False, )
    ...
    ##########################################################################
    legal_fecha_inicial = fields.Date(string="Fecha Inicial Legal",
                                      required=False, )
    legal_persona_legal = fields.Many2one('res.users',
                                          string='Documentos revisados por:',
                                          index=True, tracking=True)
    ...
    ##########################################################################
    economia_fecha_inicial = fields.Date(string="Fecha Inicial Económica",
                                         required=False, )
    economia_persona_economia = fields.Many2one('res.users',
                                                string='Aprobado por:',
                                                index=True, tracking=True)
    economia_detalles_contrato = fields.Html(string='Detalles Economía')
    economia_fecha_fin = fields.Date(string="Fecha Fin Economía",
                                     required=False, )
    ...
    forma_pago = fields.Many2many('sicpro.app.contratos.economia.pago',
                                  'sicpro_app_contratos_forma_pago_rel',
                                  string='Forma de Pago')
    # Tiempo en dias para el pago
    plazo_pago = fields.Integer(string='Plazo de Pago', tracking=False, )
    beneficiario_nombre = fields.Many2one('sicpro.app.contratos.beneficiarios',
                                          string='Beneficiario', tracking=True,
                                          required=False)

    beneficiario_moneda = fields.Many2one('res.currency', string='Moneda',
        required=False, related='beneficiario_nombre.moneda', store=True)
    ...
    ##########################################################################
    economia_dc_fecha_inicial = fields.Date(string="Fecha Inicial EDC",
                                            required=False, )
    economia_dc_persona_economia = fields.Many2one('res.users',
                                                   string='Apobado por:',
                                                   index=True, tracking=True)
    economia_dc_detalles_contrato = fields.Html(string='Detalles DCE')
    economia_dc_fecha_fin = fields.Date(string="Fecha Fin DCE",
                                        required=False, )
    ...
    ##########################################################################
    fecha_firma_director_inicial = fields.Date(string="Fecha Inicial FD",
                                               required=False, )
    fecha_firmado_director = fields.Date(string="Fecha de Firma Director",
                                         required=False, )
    fecha_firma_director_fin = fields.Date(string="Fecha Fin FD",
                                           required=False, )
    firma_director_persona = fields.Many2one('res.users',
                                             string='Firma validado por:',
                                             index=True, tracking=True)
    observaciones_firma_director = fields.Html(string="Detalles Director",
                                               required=False, )
    ...
    ##########################################################################
    fecha_firma_proveedor_inicial = fields.Date(string="Fecha Inicial FP",
                                                required=False, )
    fecha_firmado_proveedor = fields.Date(string="Fecha de Firma Proveedor",
                                          required=False, )
    fecha_firma_proveedor_fin = fields.Date(string="Fecha Fin FP",
                                            required=False, )
    firma_proveedor_persona = fields.Many2one('res.users',
                                              string='Proveedor validado por:',
                                              index=True, tracking=True)
    observaciones_firma_proovedor = fields.Html(string="Detalles Proveedor",
                                                required=False, )
    ...
    devuelto_persona_devuelve = fields.Many2one('res.users',
                                                string='Devuelto por:',
                                                index=True, tracking=True)
    ...
    fecha_devolucion_fin = fields.Date(string="Fecha fin Devolución",
                                       required=False, )
    ...
    cancelado_persona_cancelado = fields.Many2one('res.users',
    ...
    fecha_cancelado = fields.Date(string='Fecha Cancelado', index=True,
                                  tracking=True, copy=False, readonly=True)
    grupo_visualizar_all = fields.Boolean(string='grupo_visualizar_all',
                                          compute='_compute_grupo_visualizar_all')
        ...
        for record in self:
            if record.presupuesto_cup == 0:
                raise ValidationError(
                    _('Debe asignar un valor de presupuesto al contrato.'))

    # verifico que el valor de la vigencia en años no sea 0
    @api.constrains('vigencia_contrato_anios')
    def _check_valor_sap_cup(self):
        for record in self:
            if record.vigencia_contrato_anios == 0:
                raise ValidationError(
                    _('Debe asignar un valor en años de vigencia del contrato.'))

    # verifica q el usuario activo pertenezca al grupo visualizar_all
    def _compute_grupo_visualizar_all(self):
        self.grupo_visualizar_all = self.env['res.users'].has_group(
            'sicpro_app_contratos.grupo_app_contratos_all')
    ...
    def action_liberar_contrato(self, ):
        if self.doc_count != 0:
            ...
            self.write({
                'contratacion_fecha_fin': fields.Date.context_today(self) + timedelta(
                    days=dias_habiles), 'estado_id': estado,
                'contratacion_fecha_inicial': fields.Date.context_today(self),
                'contratacion_dias_habiles': dias_habiles,
                'estado_interno': 'liberado', })
            ...
            template = self.env.ref(
                'sicpro_app_contratos.contratos_contrato_cambio_estados')
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envío la notificación a los seguidores
            self.message_post(body='Contrato Liberado',
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info('[%s] Cambio de estado: Contrato Liberado - '
    ...
    def action_contratacion_contrato(self, ):
        if self.acta and self.fecha_comite_contratacion:
            ...
            dias_habiles = self.env['sicpro.app.contratos.dias'].search(
                [('name', '=', 'revision')]).valor
            self.write({'revisado_fecha_fin': fields.Date.context_today(self) + timedelta(
                days=dias_habiles), 'estado_id': estado,
                        'revisado_fecha_inicial': fields.Date.context_today(self),
                        'revisado_dias_habiles': dias_habiles,
                        'contratacion_persona_contratacion': self.env.uid,
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info('[%s] Cambio de estado: Contrato aprobado en el '
    ...
    def action_revision_contrato(self, ):
        ...
        dias_habiles = self.env['sicpro.app.contratos.dias'].search(
            [('name', '=', 'legal')]).valor
        self.write({'legal_fecha_fin': fields.Date.context_today(self) + timedelta(
            days=dias_habiles), 'estado_id': estado,
                    'legal_fecha_inicial': fields.Date.context_today(self),
                    'legal_dias_habiles': dias_habiles,
                    'revisado_persona_revisa': self.env.uid,
                    'estado_interno': 'revisado', })
        # mantiene actualizado el correo de los seguidores del registro
        correos = ''
        for follower in self.message_partner_ids:
            correos = str(correos) + str(follower.email_formatted)
        ...
        template.with_context(local_context).send_mail(self.id,
                                                       force_send=True)
        # envió la notificación a los seguidores
        self.message_post(body='Contrato aprobado en su revisión',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info('[%s] Cambio de estado: Contrato aprobado en su '
                         'revisión - Records: %s (%s) - %s. Usuario: %s - ' % tuple(
            data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
            'sicpro_app_contratos.contratos_gestion_action').read()[0]
    ...
    def action_legal_contrato(self, ):
        ...
        dias_habiles = self.env['sicpro.app.contratos.dias'].search(
            [('name', '=', 'economia')]).valor
        self.write({'economia_fecha_fin': fields.Date.context_today(self) + timedelta(
            days=dias_habiles), 'estado_id': estado,
                    'economia_fecha_inicial': fields.Date.context_today(self),
                    'economia_dias_habiles': dias_habiles,
                    'legal_persona_legal': self.env.uid,
                    'estado_interno': 'legal', })
        # mantiene actualizado el correo de los seguidores del registro
        correos = ''
        for follower in self.message_partner_ids:
            correos = str(correos) + str(follower.email_formatted)
        ...
        template.with_context(local_context).send_mail(self.id,
                                                       force_send=True)
        # envió la notificación a los seguidores
        self.message_post(body='Contrato aprobado por Legal',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info(
            '[%s] Cambio de estado: Contrato aprobado por Legal - '
    ...
    def action_economia_contrato(self, ):
        if self.forma_pago and self.plazo_pago and self.beneficiario_nombre and self.beneficiario_moneda and self.beneficiario_cuenta:
            estado = self.env['sicpro.app.contratos.estados'].search(
                [('is_economia_dc', '=', True)]).id
            dias_habiles = self.env['sicpro.app.contratos.dias'].search(
                [('name', '=', 'economia_dc')]).valor
            self.write({
                'economia_dc_fecha_fin': fields.Date.context_today(self) + timedelta(
                    days=dias_habiles), 'estado_id': estado,
                'economia_dc_fecha_inicial': fields.Date.context_today(self),
                'economia_dc_dias_habiles': dias_habiles,
                'economia_persona_economia': self.env.uid,
                'estado_interno': 'economia', })
            # mantiene actualizado el correo de los seguidores del registro
            correos = ''
            for follower in self.message_partner_ids:
                correos = str(correos) + str(follower.email_formatted)
            ...
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envío la notificación a los seguidores
            self.message_post(body='Contrato aprobado por Economía',
                              message_type='notification',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Registro del log
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info(
                '[%s] Cambio de estado: Contrato aprobado por Economía - '
                'Records: %s (%s) - %s. Usuario: %s - ' % tuple(data_log))
            # redirecciono la salida
            action = self.sudo().env.ref(
                'sicpro_app_contratos.contratos_gestion_action').read()[0]
            ...
            raise UserError(
                _('Debe proporcionar los datos de la sección de Economía,'
                  ' verifíquelo '))
    ...
    def action_economia_dc_contrato(self, ):
        ...
        self.write({
            'fecha_firma_director_fin': fields.Date.context_today(self) + timedelta(
                days=dias_habiles), 'estado_id': estado,
            'fecha_firma_director_inicial': fields.Date.context_today(self),
            'firma_director_dias_habiles': dias_habiles,
            'economia_dc_persona_economia': self.env.uid,
            'estado_interno': 'economia_dc', })
        # mantiene actualizado el correo de los seguidores del registro
        correos = ''
        for follower in self.message_partner_ids:
            correos = str(correos) + str(follower.email_formatted)
        ...
        template.with_context(local_context).send_mail(self.id,
                                                       force_send=True)
        # envió la notificación a los seguidores
        self.message_post(body='Contrato aprobado por la DC. Economía',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info(
            '[%s] Cambio de estado: Contrato aprobado por la DC. Economía - '
            'Records: %s (%s) - %s. Usuario: %s - ' % tuple(data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
            'sicpro_app_contratos.contratos_gestion_action').read()[0]
    ...
    def action_firma_director_contrato(self, ):
        if self.fecha_firmado_director:
            ...
            self.write({
                'fecha_firma_proveedor_fin': fields.Date.context_today(self) + timedelta(
                    days=dias_habiles),
                'estado_id': estado,
                'fecha_firma_proveedor_inicial': fields.Date.context_today(self),
                'firma_proveedor_dias_habiles': dias_habiles,
                'firma_director_persona': self.env.uid,
            ...
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envió la notificación a los seguidores
            self.message_post(body='Contrato firmado por el Director',
                              message_type='notification',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Registro del log
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info(
                '[%s] Cambio de estado: Contrato firmado por el Director - '
                'Records: %s (%s) - %s. Usuario: %s - ' % tuple(data_log))
            # redirecciono la salida
            action = self.sudo().env.ref(
                'sicpro_app_contratos.contratos_gestion_action').read()[0]
        ...
        if self.fecha_firmado_proveedor:
            ...
            self.write({'fecha_fin_contrato': fields.Date.context_today(self) + timedelta(
                days=self.vigencia_contrato_anios * 365), 'estado_id': estado,
                        ...
                        'firma_proveedor_persona': self.env.uid,
                        'kanban_state': 'done', 'estado_interno': 'activo', })
            # mantiene actualizado el correo de los seguidores del registro
            correos = ''
            for follower in self.message_partner_ids:
                correos = str(correos) + str(follower.email_formatted)
            ...
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envió la notificación a los seguidores
            self.message_post(body='Contrato firmado por el Proveedor',
                              message_type='notification',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Registro del log
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info(
                '[%s] Cambio de estado: Contrato firmado por el Proveedor - '
                'Records: %s (%s) - %s. Usuario: %s - ' % tuple(data_log))
            # redirecciono la salida
            action = self.sudo().env.ref(
                'sicpro_app_contratos.contratos_gestion_action').read()[0]
            ...
            rainbow = {'effect': {'fadeout': 'slow',
                                  'message': 'Felicidades. El contrato esta activo',
                                  'type': 'rainbow_man', }}
    ...
    def action_terminar_contrato(self, ):
        ...
        template.with_context(local_context).send_mail(self.id,
                                                       force_send=True)
        # envió la notificación a los seguidores
        self.message_post(body='Contrato Terminado',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info('[%s] Cambio de estado: Contrato Terminado - '
                         'Records: %s (%s) - %s. Usuario: %s - ' % tuple(
            data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
    ...
    def action_reiniciar_contrato(self, ):
        ...
        self.write({'estado_interno': 'nuevo', 'estado_id': estado,
                    'kanban_state': 'normal',
                    'solicitud_fecha_inicial': fields.Date.context_today(self),
                    'solicitud_dias_habiles': dias_habiles,
                    'solicitud_fecha_fin': fields.Date.context_today(self) + timedelta(
                        days=dias_habiles),
                    'solicitud_persona_solicita': self.env.uid,
                    'fecha_inicio_contrato': '', 'fecha_fin_contrato': '',
                    'acta': '', 'fecha_comite_contratacion': '',
                    'contratacion_persona_contratacion': False,
                    'contratacion_fecha_inicial': '',
                    'contratacion_fecha_fin': '',
                    'acuerdo_comite_contratacion': '',
                    'revisado_persona_revisa': False,
                    'revisado_fecha_inicial': '', 'revisado_fecha_fin': '',
                    'revisado_detalles_contrato': '',
                    'legal_persona_legal': False, 'consecutivo': '',
                    'legal_fecha_inicial': '', 'legal_fecha_fin': '',
                    'legal_detalles_contrato': '',
                    'economia_persona_economia': False,
                    'economia_fecha_inicial': '', 'economia_fecha_fin': '',
                    'economia_detalles_contrato': '',
                    'economia_dc_persona_economia': False,
                    'economia_dc_fecha_inicial': '',
                    'economia_dc_fecha_fin': '',
                    'economia_dc_detalles_contrato': '',
                    'firma_director_persona': False,
                    'fecha_firma_director_inicial': '',
                    'fecha_firma_director_fin': '',
                    'fecha_firmado_director': '',
                    'observaciones_firma_director': '',
                    'firma_proveedor_persona': False,
                    'fecha_firma_proveedor_inicial': '',
                    'fecha_firma_proveedor_fin': '',
                    'fecha_firmado_proveedor': '',
                    'observaciones_firma_proovedor': '',
                    'esta_rechazada': False, 'esta_cancelado': False, })
        # mantiene actualizado el correo de los seguidores del registro
        correos = ''
        for follower in self.message_partner_ids:
            correos = str(correos) + str(follower.email_formatted)
        ...
        self.message_post(body='Contrato Reiniciado',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info('[%s] Cambio de estado: Contrato Reiniciado - '
                         'Records: %s (%s) - %s. Usuario: %s - ' % tuple(
            data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
        ...
        for data in self:
            ...
            if data.estado_interno == 'nuevo':
                if hoy > data.solicitud_fecha_fin:
                    ...
                    data.sudo().dias_desfasados = str(
                        dias) + " días desfasados, Etapa: " + str(
                        data.estado_id.name)
            ...
            if data.estado_interno == 'revisado':
                if hoy > data.legal_fecha_fin:
                    ...
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de legal
            ...
            if data.estado_interno == 'legal':
                if hoy > data.economia_fecha_fin:
                    ...
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de economia

                # Realiza el cálculo en el estado de dc economia
            if data.estado_interno == 'economia':
                if hoy > data.economia_dc_fecha_fin:
                    dias = abs(hoy - data.economia_dc_fecha_fin).days
                    data.sudo().dias_desfasados = str(
                        dias) + " días desfasados, Etapa: " + str(
                        data.estado_id.name)
                    data.sudo().dias_desfasados_valor = dias
                else:
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de dc economia
            ...
            if data.estado_interno == 'economia_dc':
                if hoy > data.fecha_firma_director_fin:
                    ...
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de firma del director
            ...
            if data.estado_interno == 'firma_director':
                if hoy > data.fecha_firma_proveedor_fin:
                    ...
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de firma del proveedor
            ...
            if data.estado_interno == 'devuelto':
                if hoy > data.fecha_devolucion_fin:
                    ...
                    data.sudo().dias_desfasados = "No existen días desfasados"  # Finaliza el cálculo en el estado de rechazo del contrato

            # Realiza el cálculo en el estado de cancelado,
            # terminado o activo del contrato
            if data.estado_interno == 'cancelado' or data.estado_interno == 'terminado' or data.estado_interno == 'activo':
                data.sudo().dias_desfasados_valor = 0
                data.sudo().dias_desfasados = "-"  # Finaliza el cálculo en el estado de cancelado,  # terminado o activo del contrato

            # Realiza el cálculo en el estado de cancelado el contrato  # if data.estado_interno == 'cancelado':  #   data.dias_desfasados_valor = 0  # data.dias_desfasados = "-"  # Realiza el cálculo en el estado de terminado el contrato  # if data.estado_interno == 'terminado':  #   data.dias_desfasados_valor = 0  # data.dias_desfasados = "-"  # Realiza el cálculo en el estado de activo  # if data.estado_interno == 'activo':  #   data.dias_desfasados = "-"  # data.dias_desfasados_valor = 0

    @api.model
    def create(self, vals):
        # Crear la secuencia de incremento para el consecutivo de los contratos
        vals['sequence_consecutivo'] = self.env['ir.sequence'].next_by_code(
            'contratos_consecutivo_incrementar')
        ...
        res['solicitud_fecha_fin'] = fields.Date.context_today(self) + timedelta(
            days=dias_habiles)

        # busco los usuarios con permisos de visualización
        visualizar = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_all').users
        gestor = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_validar_gestor').users
        contratacion = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_validar_comite_contratacion').users
        revision = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_validar_area').users
        legal = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_validar_legal').users
        economia = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_validar_economica').users
        ...
        # envió la notificación a los seguidores
        res.message_post(body='Nueva Contrato Creado',
                         message_type='notification',
                         subtype_xmlid='mail.mt_comment',
                         author_id=self.env.user.partner_id.id)
        # mantiene actualizado el correo de los seguidores del registro
        ...
        # envío el correo a los seguidores del registro
        template = self.env.ref(
            'sicpro_app_contratos.contratos_contrato_nuevos')
...
class ContratoRechazado(models.TransientModel):
    ...
    def action_motivo_rechazo(self):
        ...
        post.message_post(body='Contrato rechazado.',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        ...
        rechazo = self.env['sicpro.app.contratos'].browse(
            self.env.context.get('active_ids'))
        dias_habiles = self.env['sicpro.app.contratos.dias'].search(
        ...
        data_log = [pid, self._name, self.id, rechazo.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info('[%s] Cambio de estado: Contrato rechazado - '
                         'Records: %s (%s) - %s. Usuario: %s - ' % tuple(
            data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
...
class ContratoCancelado(models.TransientModel):
    ...
    def action_motivo_cancelado(self):
        ...
        post = self.env['sicpro.app.contratos'].browse(
            self.env.context.get('active_ids'))
        post.message_post(body='Contrato cancelado.',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # cambio el estado interno del contrato
        ...
        cancelado = self.env['sicpro.app.contratos'].browse(
            self.env.context.get('active_ids'))
        ...
        logger = SmileDBLogger(self._cr.dbname, self._name, self.id, self._uid)
        pid = os.getpid()
        data_log = [pid, self._name, self.id, cancelado.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info('[%s] Cambio de estado: Contrato cancelado - Records:'
...
class ContratosCuentas(models.Model):
    ...
    name = fields.Many2one('sicpro.app.contratos', 'Contratos', required=False,
                           index=True)
    cuenta_gasto = fields.Many2one('sicpro.nomenclador.cuentas.contables',
                                   string='Cuenta de Gastos')
    cuenta_gasto_name = fields.Char(related='cuenta_gasto.descripcion',
                                    store=True)
    tipo_moneda = fields.Many2one('res.currency', string='Moneda', )
    monto_disponible = fields.Monetary(string='Monto disponible',
                                       tracking=True,
