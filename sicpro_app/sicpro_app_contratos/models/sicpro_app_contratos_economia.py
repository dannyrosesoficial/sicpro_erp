...
from odoo.addons.sicpro_app_administracion.models.monto2texto import \
    Monto2Texto
...
class ContratosEconomia(models.Model):
    ...
    def _get_default_stage_ids(self):
        return self.env['sicpro.app.contratos.economia.estados'].search([],
                                                                        limit=1)

    name = fields.Many2one('sicpro.app.contratos', 'Contratos', tracking=True,
                           required=True, index=True,
                           domain="[('estado_id.is_won', '=', True)]")
    facturas_id = fields.Many2one(
        comodel_name="sicpro.app.contratos.economia.facturas",
        string="Facturación", required=False)
    facturas_ids = fields.One2many('sicpro.app.contratos.economia.facturas',
                                   'economia', 'Facturaciónes', )
    active = fields.Boolean(string="Activo", default=True, tracking=True, )
    prioridad = fields.Selection(Prioridades_Activas,
                                 string='Prioridad Económica', index=True,
                                 tracking=True,
                                 default=Prioridades_Activas[0][0])
    estado_id = fields.Many2one('sicpro.app.contratos.economia.estados',
    ...
    etiquetas = fields.Many2many('sicpro.app.contratos.economia.etiquetas',
                                 'sicpro_app_contratos_economia_etiquetas_rel',
                                 string='Etiqueta')
    anio = fields.Char(string="Año", required=False,
                       default=fields.Datetime.now().strftime("%Y"), )
    doc_count = fields.Integer(
        compute='_compute_contratos_economia_docs_count', string="Documentos")
    facturas_count = fields.Integer(
        compute='_compute_contratos_economia_facturas_count',
        string="Facturas")
    monto_facturado = fields.Monetary(currency_field='company_currency',
                                      string="Facturado",
                                      compute='_compute_contratos_economia_suma_montos')
    monto_texto = fields.Char(string='Monto en Texto', compute='_monto_texto')
    tipo_facturacion = fields.Selection(string='Tipo de facturación',
                                        required=True,
                                        selection=[('factura', 'Factura'), (
                                            'prefactura', 'Prefactura'), ], )
    estado_interno = fields.Selection(
        [('nuevo', 'nuevo'), ('sip', 'sip'), ('emision', 'emision'),
         ('transito', 'transito'), ('terminado', 'Terminado')], index=True,
        required=True, default=lambda self: 'nuevo')
    banco_dvpe = fields.Many2one(
        comodel_name='sicpro.app.contratos.economia.bancos',
        string='Banco DVPE', required=False,
        domain="[('tipo_banco', '=', 'dvpe')]")
    cuentas_banco_dvpe = fields.Many2many(
        'sicpro.app.contratos.economia.cuentas.dvpe',
        'sicpro_app_contratos_bancos_cuentas_dvpe_rel',
        related='banco_dvpe.cuenta_dvpe')
    cuenta_dvpe = fields.Many2one('sicpro.app.contratos.economia.cuentas.dvpe',
                                  string='Nombre DVPE',
                                  domain="[('id', 'in',cuentas_banco_dvpe)]",
                                  required=False)
    numero_cuenta_dvpe = fields.Char(string='Cuenta DVPE',
                                     compute='_onchange_cuenta_dvpe')
    tipo_banco_dvpe = fields.Selection(string='Tipo de Banco DVPE', store=True,
                                       related='banco_dvpe.filtro_banco', )
    banco_beneficiario = fields.Many2one(
        comodel_name='sicpro.app.contratos.economia.bancos',
        string='Banco Beneficiario', required=False,
        domain="[('tipo_banco', '=', 'beneficiario')]")
    cuentas_banco_beneficiario = fields.Many2many(
    ...
    cuenta_beneficiario = fields.Many2one('sicpro.app.contratos.beneficiarios',
                                          string='Nombre Beneficiario',
                                          required=False)  # domain="[('id', 'in',cuentas_banco_beneficiario)]",
    numero_cuenta_beneficiario = fields.Char(string='Cuenta Beneficiaro',
                                             compute='_onchange_cuenta_beneficiario')

    ###########################################################################
    # Rol_sp
    sequence_consecutivo = fields.Char(string='Secuencia del contrato',
                                       copy=False, readonly=True, )
    sp_fecha_solicitud = fields.Date(string='Fecha de Solicitud',
                                     default=lambda self: fields.Date.context_today(self))
    sol_pago_area = fields.Many2one('sicpro.app.contratos.areas', store=True,
    ...
    sol_pago_cuenta_gasto = fields.Many2one(
        'sicpro.nomenclador.cuentas.contables', string='Cuenta de Gastos',
        tracking=True)
    sol_pago_cuenta_gasto_name = fields.Char(
        related='sol_pago_cuenta_gasto.descripcion', store=True)
    sol_pago_centro_costo = fields.Many2one('sicpro.nomenclador.centro.costo',
    ...
    sol_pago_concepto = fields.Text(string="Concepto de pago",
                                    required=False, )
    sol_pago_observaciones = fields.Text(string="Observaciones SP",
                                         required=False, )
    sol_pago_cumplimiento_cronograma = fields.Selection(
    ...
    sip_sequence_consecutivo = fields.Char(string='Secuencia SIP', copy=False,
                                           readonly=True, )
    sip_beneficiario = fields.Many2one('sicpro.app.contratos.beneficiarios',
                                       string='Beneficiario', tracking=True,
                                       required=False)
    sip_beneficiario_cuenta = fields.Char(string='Cuenta Beneficiario',
        tracking=True, related='sip_beneficiario.cuenta_beneficiario',
        store=True)
    sip_fecha_emision = fields.Date(string='Emisión SIP',
                                    default=lambda self: fields.Date.context_today(self))
    sip_indicador = fields.Many2one('sicpro.app.contratos.economia.indicador',
                                    string='Indicador', tracking=True, )
    sip_acapite = fields.Many2one('sicpro.app.contratos.economia.acapite',
                                  string='Acápite', tracking=True, )
    sip_observaciones = fields.Html(string="Observaciones SIP", store=True)
    ###########################################################################
    # accion Rol_ip
    emision_sequence_consecutivo = fields.Char(string='No.', copy=False,
    ...
    emision_motivo_cancelacion = fields.Text(
        string="Motivo de Cancelación de la Emisión")
    emision_cancelada_accion = fields.Selection(string='Acción al Cancelar',
                                                required=False, selection=[
            ('emitir', 'Emitir'), ('no_emitir', 'No Emitir'), ], )
    emision_debitar = fields.Selection(string='Debitar', required=False,
        selection=[('transferir', 'Transferir'),
    ...
    emision_pago_bfi = fields.Selection(string='Pago BFI', required=False,
        selection=[('bfi_extranjero', 'TRANSFERENCIA AL EXTRANJERO'),
                   ...
                   ('otra', 'Otra'), ], )
    emision_beneficiario_bfi = fields.Selection(string='Beneficiario BFI',
        required=False,
    ...
    transito_estado_cuenta = fields.Date(string='Estado Cuenta',
                                         tracking=True, )
    ...
    ###########################################################################
    sequence_consecutivo_contratos = fields.Char(string='Contrato', store=True,
                                                 related='name.sequence_consecutivo')

    proveedor = fields.Many2one('sicpro.app.contratos.proveedores', store=True,
                                string='Proveedor', related='name.proveedor')
    tipo_contrato = fields.Many2one('sicpro.app.contratos.tipo', string='Tipo',
                                    related='name.tipo_contrato', store=True)
    pep = fields.Char(string="Código SAP Contrato", related='name.pep',
                      store=True)
    solicitud_fecha_inicial = fields.Date(store=True,
                                          string="Solicitud Fecha Inicial",
                                          related='name.solicitud_fecha_inicial')
    solicitud_persona_solicita = fields.Many2one('res.users',
        string='Solicitado por:', related='name.solicitud_persona_solicita',
        store=True)
    solicitud_fecha_fin = fields.Date(string="Solicitud Fecha Fin", store=True,
                                      related='name.solicitud_fecha_fin')
    ...
    fecha_comite_contratacion = fields.Date(string="Fecha del Comité",
                                            related='name.fecha_comite_contratacion',
                                            store=True, )
    forma_pago_contratos = fields.Many2many(
        'sicpro.app.contratos.economia.pago',
        'sicpro_app_contratos_forma_pago_rel', related='name.forma_pago',
        string='Forma de Pago')
    forma_pago = fields.Many2one('sicpro.app.contratos.economia.pago',
                                 string='Método de Pago', required=False,
                                 domain="[('id', '=', forma_pago_contratos)]")
    forma_pago_genera_consecutivo = fields.Boolean()

    plazo_pago = fields.Integer(string='Plazo de Pago',
                                related='name.plazo_pago', store=True)
    presupuesto_cup = fields.Monetary('Presupuesto', store=True,
                                      currency_field='company_currency',
                                      related='name.presupuesto_cup')
    vigencia_contrato_anios = fields.Integer(string='Vigencia (Años)',
                                             related='name.vigencia_contrato_anios',
                                             store=True)
    uodc = fields.Many2one('sicpro.app.contratos.unidades', string='Unidad',
                           related='name.uodc', store=True)
    area_contrato = fields.Many2one('sicpro.app.contratos.areas', store=True,
                                    string='Área del contrato',
                                    related='name.area_contrato')
    prioridad_contrato = fields.Selection(string='Prioridad contrato',
                                          related='name.prioridad', store=True)
    fecha_inicio_contrato = fields.Date(string="Inicio del Contrato",
                                        related='name.fecha_inicio_contrato',
                                        store=True)
    fecha_fin_contrato = fields.Date(string="Fin del Contrato",
                                     related='name.fecha_fin_contrato',
                                     store=True)
    moneda = fields.Many2one('res.currency', string='Moneda', required=False)
    ###########################################################################
    # Campos heredados del modelo de proveedores
    ###########################################################################
    domicilio_social = fields.Char(string='Domicilio', store=True,
                                   related='name.proveedor.domicilio_social')
    nombre_directivo = fields.Char(string='Nombre Directivo', store=True,
                                   related='name.proveedor.nombre_directivo')
    cargo_directivo = fields.Char(string='Cargo', store=True,
                                  related='name.proveedor.cargo_directivo')
    telefono_fijo = fields.Char(string="Teléfono", store=True,
                                related='name.proveedor.telefono_fijo')
    telefono_movil = fields.Char(string="Móvil", store=True,
                                 related='name.proveedor.telefono_movil')
    correo = fields.Char(string="Correo electrónico",
                         related='name.proveedor.correo', store=True)
    codigo_reeup = fields.Char(string="Código REEUP / ONE", store=True,
                               related='name.proveedor.codigo_reeup')
    servicio_comercializable = fields.Text(
        related='name.proveedor.servicio_comercializable', store=True,
        string="Servicio Comercial")
    pep_proveedor = fields.Char(string="Código SAP Proveedor",
                                related='name.proveedor.pep', store=True)
    tipo = fields.Many2one(
        comodel_name='sicpro.app.contratos.proveedores.tipo', string='Tipo P.',
        related='name.proveedor.tipo', store=True)
    fecha_agregado = fields.Date(string="Agregado", store=True,
                                 related='name.proveedor.fecha_agregado')
    ...
    digito_10_cuenta_beneficiario = fields.Char(
        string='digito_10_beneficiario')
    digito_11_cuenta_beneficiario = fields.Char(
        string='digito_11_beneficiario')
    ...
    def _compute_contratos_economia_docs_count(self):
        ...
        for documentos in self:
            documentos.doc_count = attachment_obj.search_count(
                ['&', ('res_model', '=', 'sicpro.app.contratos.economia'),
                 ('res_id', '=', documentos.id)])

    # Cuenta las facturas o prefacturas
    def _compute_contratos_economia_facturas_count(self):
        ...
        for facturacion in self:
            facturacion.facturas_count = data.search_count(
                [('economia', '=', self._origin.id)])
    ...
    def action_sip(self, ):
        if self.facturas_count > 0:
            estado = self.env['sicpro.app.contratos.economia.estados'].search(
                [('is_instrumento', '=', True)]).id
            bancos = self.env['sicpro.app.contratos.economia.bancos'].search(
                [('cuenta_beneficiario', 'in', self.name.beneficiario_nombre.id)])
            banco = 0
            for item in bancos:
                banco = item.id
            self.write({'estado_interno': 'sip', 'estado_id': estado,
                        'sip_beneficiario': self.name.beneficiario_nombre,
                        'sip_beneficiario_cuenta': self.name.beneficiario_cuenta,
                        'banco_beneficiario': banco, })

            # Crear la secuencia de incremento para el consecutivo del
            # instrumento de pago
            self.sip_sequence_consecutivo = self.env[
            ...
            template = self.env.ref(
                'sicpro_app_contratos.contratos_economia_cambio_estados')
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envío la notificación a los seguidores
            self.message_post(body='Instrumento de Pago solicitado',
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info(
                '[%s] Cambio de estado: Instrumento de Pago solicitado - Records: '
                '%s (%s) - %s. Usuario: %s - ' % tuple(data_log))
            # redirecciono la salida
            action = self.sudo().env.ref(
                'sicpro_app_contratos.contratos_economia_gestion_action').read()[
                0]
            return action
        else:
            raise UserError(
                _('Debe agregar Facturas / Prefacturas para poder continuar, '
                  'verifíquelo.'))

    # acción para realizar la emision de pago
    def action_emision(self, ):
        if self.forma_pago and self.sip_beneficiario:
            ...
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envío la notificación a los seguidores
            self.message_post(body='Emisión de Pago solicitado',
                              message_type='notification',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Registro del log
    ...
    # acción para pasar la solicitud de pago a transito
    def action_transito(self, ):
        if self.emision_fecha and self.banco_dvpe and self.cuenta_dvpe and self.banco_beneficiario and self.sip_beneficiario_cuenta:
            estado = self.env['sicpro.app.contratos.economia.estados'].search(
                [('transito', '=', True)]).id
            self.write({'estado_interno': 'transito', 'estado_id': estado,
            ...
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True)
            # envío la notificación a los seguidores
            self.message_post(body='Solicitud de Pago en transito',
                              message_type='notification',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Registro del log
            ...
            data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                        self.env.user.name]
            logger.time_info(
                '[%s] Cambio de estado: Solicitud de Pago en transito - Records: '
                '%s (%s) - %s. Usuario: %s - ' % tuple(data_log))
            # redirecciono la salida
            action = self.sudo().env.ref(
                'sicpro_app_contratos.contratos_economia_gestion_action').read()[
            ...
            raise UserError(
                _('Existen campos de la emisión de pago sin completar,'
    ...
    def action_terminado(self, ):
        ...
        template.with_context(local_context).send_mail(self.id,
                                                       force_send=True)
        # envío la notificación a los seguidores
        self.message_post(body='Solicitud de Pago terminada',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Registro del log
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info(
            '[%s] Cambio de estado: Solicitud de Pago terminada - Records: '
            '%s (%s) - %s. Usuario: %s - ' % tuple(data_log))
        # redirecciono la salida
        action = self.sudo().env.ref(
            'sicpro_app_contratos.contratos_economia_gestion_action').read()[0]

        rainbow = {'effect': {'fadeout': 'slow',
                              'message': 'Felicidades. Las operaciones han sido validadas '
                                         'correctamente',
                              'type': 'rainbow_man', }}
        return rainbow
    ...
    def action_reemitir_emision(self, ):
        ...
        data_log = [pid, self._name, self.id, self.sequence_consecutivo,
                    self.env.user.name]
        logger.time_info(
            '[%s] Acción Ejecutada: Se re-emitido la documentación de la '
            'transferencia - Records: %s (%s) - %s. Usuario: %s - ' % tuple(
                data_log))
        # Crear la secuencia de incremento para el
        # consecutivo de la emisión de pago
        self.emision_sequence_consecutivo = self.env[
    ...
    # verifica el cambio del banco de la dvpe
    ...
    # verifica el cambio del banco del beneficiario
    ...
    # verifica si la forma de pago seleccionada genera id de consecutivo
    ...
    # Relleno los digitos de la cuenta de la DVPE
    ...
    def _onchange_emision_id_bfi(self):
        ...
        if len(digit) == 11:
            ...
            self.emision_id_bfi = ''
            # raise UserError(_('El campo ID BFI esta vacío o no contiene todos
            # los caracteres requeridos (11), verifíquelo.'))

    # acción para emitir la transferencia del banco metropolitano
    def emitir_transferencia_banco_metropolitano(self):
        return {'type': 'ir.actions.report',
                'model': 'sicpro.app.contratos.economia',
    ...
    def create(self, vals):
        ...
        res['sequence_consecutivo'] = self.env['ir.sequence'].next_by_code(
            'contratos_economia_consecutivo_incrementar')

        # busco los usuarios con permisos de visualización
        visualizar = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_economia_visual').users
        sp = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_economia_operaciones_sp').users
        sip = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_economia_operaciones_sip').users
        ep = self.env.ref(
            'sicpro_app_contratos.grupo_app_contratos_economia_operaciones_ip').users
        ...
        res.message_post(body='Nueva SP Creada',
                         message_type='notification',
                         subtype_xmlid='mail.mt_comment',
                         author_id=self.env.user.partner_id.id)
        # mantiene actualizado el correo de los seguidores del registro
        ...
        # envío el correo a los seguidores del registro
        template = self.env.ref(
            'sicpro_app_contratos.contratos_economia_nuevos')
...
class ContratosEconomiaFacturas(models.Model):
    ...
    ##########################################################################
    factura_numero = fields.Char(string='Factura', tracking=True,
                                 required=True)
    factura_Proveedor = fields.Many2one('sicpro.app.contratos.proveedores',
                                        string='Proveedor', tracking=True, )
    factura_fecha_emision = fields.Date(string='Emisión de Factura',
                                        tracking=True, )
    factura_fecha_recogida = fields.Date(string='Recibida y Aceptada',
                                         tracking=True, )
    factura_tipo_moneda = fields.Many2one('res.currency', string='Moneda', )
    factura_monto = fields.Monetary(string='Monto', tracking=True,
                                    required=True,
                                    currency_field='factura_tipo_moneda', )
    economia = fields.Many2one('sicpro.app.contratos.economia', 'economia',
                               index=True, )
    ...
    def _onchange_factura_fecha_recogida(self):
        if self.factura_fecha_recogida < self.factura_fecha_emision:
            ...
            raise UserError(
                _('La fecha de recogida de la factura no puede ser menor que '
    ...
    def _onchange_factura_numero(self):
        ...
        data = self.env['sicpro.app.contratos.economia'].search(
            ['&', ('facturas_ids', '=', facturas.id),
             ('emision_aceptada_fecha', '!=', False), ])

        if data:
            raise UserError(
                _("Esta factura perteneciente al proveedor: " +
                  str
                    (self.factura_Proveedor.name) +
                  " ya ha sido pagada, verifíquelo."))
