odoo.define('sicpro_app_contratosreconciliation_tests.data', function () {
"use strict";
...
var db = {
    ...
    'res.partner': {
        fields: {
            ...
            property_account_receivable_id: {string: 'Account receivable', type: 'many2one', relation: 'sicpro.app.contratos'},
            property_account_payable_id: {string: 'Account payable', type: 'many2one', relation: 'sicpro.app.contratos'},
        },
        records: [
    ...
    'sicpro.app.contratos.etiquetas':{
        fields: {
            id: {string: "ID", type: 'integer'},
        },
        records: [
    ...
    'sicpro.app.contratos.tax.reparticion': {
        fields: {
            id: {string: "ID", type: 'integer'},
            repartition_type: {string:"Repartition Type", type: 'selection'},
            account_id: {string: "Account", type: 'many2one', relation: 'sicpro.app.contratos'},
            factor_percent: {string: "%", type: 'integer'},
            tag_ids: {string:"Tax Grids", type: 'many2many', relation: 'sicpro.app.contratos.etiquetas'}
        },
        records: [
    ...
    'sicpro.app.contratos.tax': {
        fields: {
            ...
            invoice_repartition_line_ids: {string: "Invoice Repartition", type: 'one2many', relation: 'sicpro.app.contratos.tax.reparticion'},
        ...
        json_friendly_compute_all: function (args) {
            var tax = _.find(db['sicpro.app.contratos.tax'].records, {'id': args[0][0]});
            var amount = args[1];
            ...
            for (let i = 0 ; i < tax.invoice_repartition_line_ids.length ; i++) {
                var rep_ln = _.find(db['sicpro.app.contratos.tax.reparticion'].records, {'id': tax.invoice_repartition_line_ids[i]});
    ...
    'sicpro.app.contratos.banco': {
        fields: {},
    },
    'sicpro.app.contratos.banco.line': {
        fields: {
            id: {string: "ID", type: 'integer'},
            display_name: {string: "Displayed name", type: 'char'},
    ...
    'sicpro.app.contratos.movimiento.line': {
        fields: {},
    },
    'sicpro.app.contratos.conciliacion.model': {
        fields: {
            id: {string: "ID", type: 'integer'},
            ...
            account_id: {string: "Account", type: 'many2one', relation:'sicpro.app.contratos'},
            journal_id: {string: "Journal", type: 'many2one', relation:'sicpro.app.contratos.diario'},
            ...
            tax_ids: {string: "Tax", type: 'many2many', relation:'sicpro.app.contratos.tax'},
            analytic_account_id: {string: "Analytic Account", type: 'many2one', relation:'account.analytic.account'},
            second_account_id: {string: "Second Account", type: 'many2one', relation:'sicpro.app.contratos', domain:[('deprecated', '=', false)]},
            second_journal_id: {string: "Second Journal", type: 'many2one', relation:'sicpro.app.contratos.diario',  help:"This field is ignored in a bank statement reconciliation."},
            ...
            second_tax_ids: {string: "Second Tax", type: 'many2many', relation: 'sicpro.app.contratos.tax'},
            second_analytic_account_id: {string: "Second Analytic Account", type: 'many2one', relation:'account.analytic.account'},
            match_journal_ids: {string: "Journal Ids", type: 'many2many', relation: 'sicpro.app.contratos.diario'},
    ...
    'sicpro.app.contratos.conciliacion': {
        fields: {},
...
var auto_reconciliation = {
    ...
    'notifications': [{
        ...
        'type': "info",
        'details': {
            'model': "sicpro.app.contratos.movimiento",
...
odoo.define('sicpro_app_contratosreconciliation_tests', function (require) {
"use strict";

var ReconciliationClientAction = require('sicpro_app_contratosReconciliationClientAction');
var ReconciliationRenderer = require('sicpro_app_contratosReconciliationRenderer');
...
QUnit.module('account', {
    ...
    QUnit.test('Reconciliation basic rendering', async function (assert) {
        ...
        testUtils.mock.addMockEnvironment(clientAction, {
            ...
            archs: {
                'sicpro.app.contratos.banco.line,false,search': '<search string="Statement Line"><field name="display_name"/></search>',
            },
        });
    ...
    QUnit.test('Reconciliation create line', async function (assert) {
        ...
        testUtils.mock.intercept(clientAction, 'do_action', function (event) {
            assert.strictEqual(JSON.stringify(event.data.action),
                '{"type":"ir.actions.act_window","res_model":"sicpro.app.contratos.conciliacion.model","views":[[false,"form"]],"target":"current"}',
        ...
        testUtils.mock.intercept(clientAction, 'do_action', function (event) {
            assert.strictEqual(JSON.stringify(event.data.action),
                '{"type":"ir.actions.act_window","res_model":"sicpro.app.contratos.conciliacion.model","views":[[false,"list"],[false,"form"]],"view_mode":"list","target":"current"}',
    ...
    QUnit.test('Reconciliation create line (many2one test)', async function (assert) {
        ...
        testUtils.mock.addMockEnvironment(clientAction, {
            ...
            archs: {
                "sicpro.app.contratos,false,list": '<tree string="Account"><field name="code"/><field name="name"/></tree>',
                "sicpro.app.contratos,false,search": '<search string="Account"><field name="code"/></search>',
                'sicpro.app.contratos.banco.line,false,search': '<search string="Statement Line"><field name="display_name"/></search>',
            },
            mockRPC: function (route, args) {
    ...
    QUnit.test('Reconciliation fetch correct reconciliation models', async function (assert) {
        ...
        testUtilsMock.patch(this.params.options.context, {
            active_model: 'sicpro.app.contratos.diario', // On account dashboard, click "Reconcile" on a journal
        ...
        testUtils.addMockEnvironment(clientAction, {
            ...
            mockRPC: async function (route, args) {
                if (args.model === 'sicpro.app.contratos.conciliacion.model' && args.method === 'search_read') {
                    assert.deepEqual(
                        args.kwargs.domain, [
                            ['company_id', 'in', [3,4]],
                            '|',
            ...
            mockRPC: function(route, args) {
                if (args.method === "name_search") {
                    switch (args.model) {
                        ...
                        case 'sicpro.app.contratos.diario':
                            assert.step("Journal");
                            return Promise.resolve(
                                _.map(this.data[args.model].records, function (record) {
                                    return [record.id, record.display_name];
