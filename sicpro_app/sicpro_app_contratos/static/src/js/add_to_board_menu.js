odoo.define('sicpro_app_contratos.AddToBoardMenu_contratos', function (require) {
    "use strict";
   ...
   // buscar el valor del permiso de usuario del dashboard
     var permiso_valor = (function(){
         rpc.query({
             model: 'sicpro.app.contratos.dashboard',
             method: 'check_user_group',
         }).then(function(result){
                  login_user = result['groups'];
                 });
     })();


    class AddToBoardMenu_contratos extends DropdownMenuItem {
        constructor() {
            super(...arguments);
         ...
         * is supposed to call the route /sicpro_app_contratos/add_to_dashboard with proper
         * information.
         * @private
         */
        async _addToBoard() {
            ...
            const result = await this.rpc({
                route: '/sicpro_app_contratos/add_to_dashboard',
                params: {
            ...
            if (result) {
                this.env.services.notification.notify({
                    title: sprintf(this.env._t("'%s' agregado al dashboard"), this.state.name),
                    message: this.env._t("Por favor refresque el navegador para que los cambios surtan efectos."),
                    type: 'warning',
                });
            } else {
                this.env.services.notification.notify({
    ...
    AddToBoardMenu_contratos.props = {};
    AddToBoardMenu_contratos.template = 'AddToBoardMenu_contratos';

    FavoriteMenu.registry.add('add-to-board-menu', AddToBoardMenu_contratos, 10);

    return AddToBoardMenu_contratos;
});
