...
class WebDatabaseHerencia(Database):
    ...
    def _render_template(self, **d):
        ...
        with file_open(
            "sicpro_modulo_web_base_datos/static/src/public/database_manager.qweb.html",
            "r") as fd:
            templates['database_manager'] = fd.read()
        with file_open(
            "sicpro_modulo_web_base_datos/static/src/public/database_manager.master_input.qweb.html",
            "r") as fd:
            templates['master_input'] = fd.read()
        with file_open(
            "sicpro_modulo_web_base_datos/static/src/public/database_manager.create_form.qweb.html",
            "r") as fd:
    ...
    # url del selector de base de datos
    @http.route('/web/database/sicpro/selector', type='http', auth="none")
    ...
    @http.route('/web/database/sicpro/manager', type='http', auth="none")
    def manager(self, **kw):
        if request.db:
            request.env.cr.close()
    ...
    def create(self, master_pwd, name, lang, password, **post):
        ...
        try:
            if not re.match(DBNAME_PATTERN, name):
                raise Exception("¡Error!: Asegúrese de utilizar "
                                "solo letras, números, guiones bajos, guiones o puntos en el nombre de la base de datos.\n\n" + MSG_SOPORTE_SICPRO)
            # country code could be = "False" which is actually True in python
            country_code = post.get('country_code') or False
            dispatch_rpc('db', 'create_database',
                         [master_pwd, name, bool(post.get('demo')), lang,
                          password, post['login'], country_code,
                          post['phone']])
            credential = {'login': post['login'], 'password': password,
                          'type': 'password'}
            ...
            error = "Error al crear la base de datos: %s" % (str(e) or repr(e))
    ...
    def duplicate(self, master_pwd, name, new_name, neutralize_database=False):
        ...
        try:
            ...
            return request.redirect('/web/database/sicpro/manager')
        except Exception as e:
            _logger.exception("Error de duplicación de base de datos.")
            error = "Error de duplicación de base de datos: %s" % (
                    str(e) or repr(e))
            return self._render_template(error=error)
    ...
    def drop(self, master_pwd, name):
        ...
        try:
            ...
            _logger.exception("Error al eliminar la base de datos.")
            error = "Error al eliminar la base de datos: %s" % (
                    str(e) or repr(e))
            return self._render_template(error=error)
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        try:
            ...
            if name not in http.db_list():
                raise Exception("Base de datos %r is not known" % name)
            ...
            dump_stream = odoo.service.db.dump_db(name, None, backup_format,
                                                  filestore)
            response = Response(dump_stream, headers=headers,
                                direct_passthrough=True)
            return response
        except Exception as e:
            _logger.exception('Database.backup')
            error = "Error de copia de seguridad de la base de datos: %s" % (
                    str(e) or repr(e))
            return self._render_template(error=error)
    ...
    def restore(self, master_pwd, backup_file, name, copy=False,
        ...
        try:
            ...
            error = "Error de restauración de base de datos: %s" % (
                    str(e) or repr(e))
            return self._render_template(error=error)
        finally:
            if data_file:
    ...
    def change_password(self, master_pwd, master_pwd_new):
        try:
            ...
            error = "Error de actualización de contraseña maestra: %s" % (
                    str(e) or repr(e))
            return self._render_template(error=error)
