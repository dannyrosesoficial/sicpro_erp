...
class AdministracionRestApi(models.Model):
    ...
    name = fields.Selection(
        selection_add=[('sicpro.modulo.calendario.sync', 'MÓDULO SYNC CALENDARIO')],
        ondelete={'sicpro.modulo.calendario.sync': 'cascade'})

    # IMPORTANTE: El nombre de la acción debe ser 'api_test_' + el nombre
    # del valor del campo name
    def api_test_sicpro_modulo_api_calendario(self):
        ...
        if response.status_code == 200:
            ...
            raise ValidationError(_('Conexión reusada en el url Data.'
                                    ' Verifíquelo'))

    # IMPORTANTE: El nombre de la acción debe ser 'api_cron_' + el nombre
    # del valor del campo name
    def api_cron_sicpro_modulo_api_calendario(self):
