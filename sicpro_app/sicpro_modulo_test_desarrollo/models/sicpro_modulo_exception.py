...
class SicproException(models.Model):
    _name = 'sicpro.modulo.test.exceptions'
    _description = 'Test Exception Model'

    def generate_redirect_warning(self):
        action = self.env.ref(
            'sicpro_modulo_test_desarrollo.action_test_exceptions')
        raise odoo.exceptions.RedirectWarning('description', action.id,
