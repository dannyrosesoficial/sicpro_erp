...
class GestorDocumentos(models.Model):
    ...
    child_ids = fields.One2many('sicpro.app.gestor.documental', 'parent_id',
                                string='Crear Carpetas')
    child_count = fields.Integer(compute='_compute_child_count',
                                 string='Cantidad')
    attachment_ids_1 = fields.Many2many('ir.attachment', string="Adjuntar")

    _sql_constraints = [
        ('parent_id_name_uniq', 'unique(parent_id, name)',
         'El nombre ya existe!'),
    ]
    ...
    def _compute_child_count(self):
        ...
        count_data = self.env[comodel_name].read_group(
            [(inverse_name, 'in', self.ids)], [inverse_name], [inverse_name])
        mapped_data = dict([(count_item[inverse_name][0],
                             count_item['%s_count' % inverse_name])
    ...
    def name_get(self):
        if self.env.context.get('display_full_name', False):
            ...
            return super(GestorDocumentos, self).name_get()
        def get_names(record):
            ...
            return res
        return [(record.id, " / ".join(reversed(get_names(record))))
                for record in self]
