...
FIELDS_BLACKLIST = ["id", "create_uid", "create_date", "write_uid",
    "write_date", "display_name", "__last_update", ]
EMPTY_DICT = {}


class DictDiffer:
    """Calcule la diferencia entre dos diccionarios como:
    (1) elementos agregados
    (2) elementos eliminados
    (3) claves iguales en ambos pero valores cambiados
    (4) claves iguales en ambos valores y sin cambios
    """
    ...
    def changed(self):
        return {o for o in self.intersect if
...
class ThrowAwayCache:
    """Administrador de contexto para leer valores usando un caché desechable.

    Esto le permite recuperar valores de campo como superusuario sin envenenar el
    caché con valores no accesibles para el usuario actual.

    También le permite recuperar valores nuevos de la base de datos sin arrojar
    elimina los valores no guardados de la caché del usuario actual durante una escritura.
    """

    # Lista de atributos del objeto de transacción que deben reservarse para
    # un borrón y cuenta nueva (temporal).
    transaction_attributes = ["field_data", "field_data_patches",
    ...
    def __enter__(self):
        """Reemplace el almacenamiento de datos caché de la transacción.

        Los entornos comparten un caché común que se almacena en varias propiedades.
        de la transacción compartida. El objeto de la transacción en sí también está vinculado.
        al cursor, por lo que si queremos seguir usando el mismo cursor, debemos
        parchear estas propiedades.
        """
        for attribute in self.transaction_attributes:
            instance = getattr(self._transaction, attribute)
            setattr(self, f"_original_{attribute}", instance, )
            # Create an empty copy of the container instance
            replacement = copy.copy(instance)
            replacement.clear()
            setattr(self._transaction, attribute, replacement, )
...
class AuditlogRule(models.Model):
    _name = "auditlog.rule"
    _description = "Registro de auditoría - Regla"

    name = fields.Char(required=True)
    model_id = fields.Many2one("ir.model", "Modelo",
        help="Seleccione el modelo para el cual desea generar el registro",
        ondelete="set null", index=True, )
    model_name = fields.Char(readonly=True)
    model_model = fields.Char(string="Nombre del modelo técnico",
                              readonly=True)
    user_ids = fields.Many2many("res.users", "audittail_rules_users",
        "user_id", "rule_id", string="Usuarios",
        help="Si no se agrega ningún usuario, se aplicará a todos los usuarios.", )
    log_read = fields.Boolean(string="Lecturas de registro", help=(
        "Seleccione esto si desea realizar un seguimiento de las lecturas/aperturas en cualquier "
        "registro del modelo de esta regla"), )
    log_write = fields.Boolean(string="Escrituras de registro", default=True, help=(
        "Seleccione esto si desea realizar un seguimiento de las modificaciones en cualquier "
        "registro del modelo de esta regla"), )
    log_unlink = fields.Boolean(string="Eliminaciones de registros", default=True,
        help=(
            "Seleccione esto si desea realizar un seguimiento de la eliminación de cualquier "
            "registro del modelo de esta regla"), )
    log_create = fields.Boolean(string="Creación de registros", default=True, help=(
        "Seleccione esto si desea realizar un seguimiento de la creación en cualquier "
        "registro del modelo de esta regla"), )
    log_export_data = fields.Boolean(string="Exportaciones de registros",
        default=True, help=(
            "Seleccione esto si desea realizar un seguimiento de las exportaciones del modelo de esta regla"), )
    log_type = fields.Selection(
        [("full", "Registro completo"), ("fast", "Registro rápido")],
        string="Tipo", required=True, default="full", help=(
            "Registro completo: se recuperan los valores anteriores de los campos actualizados y "
            "almacenado en las líneas de registro. Esto incluye los campos calculados almacenados en "
            "los registros actualizados que pueden verse afectados por los nuevos valores de otros"
            "campos.\n"
            "Registro rápido: registra solo los nuevos valores asignados mediante la creación y "
            "Operaciones de escritura (menos información, pero es más rápida)"), )

    state = fields.Selection(
        [("draft", "Borrador"), ("confirmed", "Confirmado")], required=True,
        default="draft", )
    action_id = fields.Many2one("ir.actions.act_window", string="Acción", )
    capture_record = fields.Boolean(string="Registrar valores al eliminar",
        help=(
            "En caso de registro completo, registre también todos los valores de los registros en el "
            "momento de eliminación"), )
    users_to_exclude_ids = fields.Many2many("res.users",
        string="Usuarios a excluir",
        help="Los cambios realizados por estos usuarios no se registran.",
        context={"active_test": False}, )

    fields_to_exclude_ids = fields.Many2many("ir.model.fields",
        domain="[('model_id', '=', model_id)]",
        help="Los cambios realizados en estos campos no se registran.",
        string="Campos para excluir", )

    _model_uniq = models.Constraint("unique(model_id)",
        "Ya existe una regla definida en este modelo.\n"
        "No puedes definir otro: edita el existente", )
    ...
    def _patch_methods(self):
        ...
        for rule in self:
            ...
            if rule.log_export_data and not hasattr(model_model, check_attr):
                updated = rule._patch_method(model_model, "export_data",
                                             check_attr)
        ...
        for rule in self:
            ...
            for method in ["create", "read", "write", "unlink", "export_data"]:
                if getattr(rule, f"log_{method}") and hasattr(
                    ...
                    setattr(type(model_model), method,
                        getattr(model_model, method).origin)
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "model_id" not in vals or not vals["model_id"]:
                raise UserError(
                    "No hay ningún modelo definido para crear línea.")
    ...
    def get_auditlog_fields(self, model):
        ...
        override this.
        """
        return list(n for n, f in model._fields.items() if
                    (not f.compute and not f.related) or f.store)
                ...
                for new_record in new_records.sudo():
                    ...
                    for fname, field in new_record._fields.items():
                        ...
                        new_values[new_record.id][
                            fname] = field.convert_to_read(new_record[fname],
                            new_record)

            if self.env.user in users_to_exclude:
                return new_records
            rule_model.sudo().create_logs(self.env.uid, self._name,
                new_records.ids, "create", None, new_values,
                {"log_type": log_type}, )
            return new_records
    ...
    def _make_write(self):
        ...
        def write_full(self, vals, **kwargs):
            ...
            records_write = (self.filtered(
                lambda r: not isinstance(r.id, NewId)).sudo().with_context(
                prefetch_fields=False))
            if not records_write:
                return write_full.origin(self, vals, **kwargs)

            with ThrowAwayCache(self.env):
                old_values = {d["id"]: d for d in
                              records_write.read(fields_list)}
            ...
            with ThrowAwayCache(self.env):
                new_values = {d["id"]: d for d in
                              records_write.read(fields_list)}

            rule_model.sudo().create_logs(self.env.uid, self._name,
                records_write.ids, "write", old_values, new_values,
                {"log_type": log_type}, )
            return result
    ...
    def _make_unlink(self):
        ...
        def unlink_full(self, **kwargs):
            ...
            fields_list = rule_model.get_auditlog_fields(self)
            old_values = {d["id"]: d for d in
                self.sudo().with_context(prefetch_fields=False).read(
                    fields_list)}
            if self.env.user in users_to_exclude:
                return unlink_full.origin(self, **kwargs)
            rule_model.sudo().create_logs(self.env.uid, self._name, self.ids,
                "unlink", old_values, None, {"log_type": log_type}, )
            return unlink_full.origin(self, **kwargs)
        ...
        def unlink_fast(self, **kwargs):
            ...
            rule_model.sudo().create_logs(self.env.uid, self._name, self.ids,
                "unlink", None, None, {"log_type": log_type}, )
            return unlink_fast.origin(self, **kwargs)
    ...
    def _make_export_data(self):
        ...
        def export_data(self, fields_to_export):
            ...
            if self.env.user in users_to_exclude:
                return res
            rule_model.sudo().create_logs(self.env.uid, self._name, self.ids,
                "export_data", None, None, {"log_type": log_type}, )
            return res
    ...
    def create_logs(self, uid, res_model, res_ids, method, old_values=None,
        ...
        vals = {"model_id": model_id, "method": method, "user_id": uid,
        ...
        for res_id in res_ids:
            ...
            diff = DictDiffer(new_values.get(res_id, EMPTY_DICT),
            ...
            if method == "create":
                ...
                log_vals["line_ids"] = self._create_log_line_on_read(log_vals,
                    list(old_values.get(res_id, EMPTY_DICT).keys()),
                    old_values, fields_to_exclude, )
            elif method == "write":
                log_vals["line_ids"] = self._create_log_line_on_write(log_vals,
                    diff.changed(), old_values, new_values, fields_to_exclude)
    ...
    def _create_log_line_on_read(self, log_vals, fields_list, read_values,
        fields_to_exclude):
        ...
        for field_name in fields_list:
            ...
            if field:
                line_vals.append(Command.create(
                    self._prepare_log_line_vals_on_read(log_vals, field,
                        read_values)))
        return line_vals
    ...
    def _prepare_log_line_vals_on_read(self, log_vals, field, read_values):
        ...
        vals = {"field_id": field["id"],
            ...
            "new_value": False, "new_value_text": False, }
        if field["relation"] and "2many" in field["ttype"]:
            vals["old_value_text"] = [(x.id, x.display_name) for x in
                self.env[field["relation"]].browse(vals["old_value"])]
        return vals

    def _create_log_line_on_write(self, log_vals, fields_list, old_values,
        new_values, fields_to_exclude):
        ...
        for field_name in fields_list:
            ...
            if field:
                line_vals.append(Command.create(
                    self._prepare_log_line_vals_on_write(log_vals, field,
                        old_values, new_values)))
        return line_vals

    def _prepare_log_line_vals_on_write(self, log_vals, field, old_values,
                                        new_values):
        """Prepare the dictionary of values used to create a log line on a
        'write' operation.
        """
        ...
        if (log_vals["log_type"] == "full" and field["relation"] and "2many" in
            field["ttype"]):
            ...
            old_value_text = []
            if existing_ids:
                old_value_text = [(x.id, x.display_name) for x in
                    self.env[field["relation"]].browse(existing_ids)]
            ...
            vals["new_value_text"] = [(x.id, x.display_name) for x in
                self.env[field["relation"]].browse(vals["new_value"])]
        return vals

    def _create_log_line_on_create(self, log_vals, fields_list, new_values,
        fields_to_exclude):
        ...
        for field_name in fields_list:
            ...
            if field:
                line_vals.append(Command.create(
                    self._prepare_log_line_vals_on_create(log_vals, field,
                        new_values)))
        return line_vals
    ...
    def _prepare_log_line_vals_on_create(self, log_vals, field, new_values):
        ...
        'create' operation.
        """
        vals = {"field_id": field["id"], "old_value": False,
    ...
    def set_to_confirmed(self):
        ...
        for rule in self:
            ...
            vals = {"name": "Ver registros de auditoría",
                    "res_model": "auditlog.log",
                    "binding_model_id": rule.model_id.id, "domain": domain, }
