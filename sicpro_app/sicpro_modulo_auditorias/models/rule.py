...
class AuditlogRule(models.Model):
    _name = "sicpro.modulo.auditorias.rule"
    _description = "Auditorias - Reglas"
    ...
    log_read = fields.Boolean(
        ...
        help=(
            ...
            "record of the models of this rule"
        ),
        states={"subscribed": [("readonly", True)]},
    )
    ...
    _sql_constraints = [
        (
            ...
            "unique(model_id)",
            (
                "There is already a rule defined on this models\n"
    ...
    def _patch_methods(self):
        ...
        for rule in self:
            ...
            # CRUD
            #   -> create
            check_attr = "sicpro_modulo_auditorias_ruled_create"
        ...
        for rule in self:
            ...
            for method in ["create", "read", "write", "unlink"]:
                if getattr(rule, "log_%s" % method) and hasattr(
                    ...
                    delattr(type(model_model), "sicpro_modulo_auditorias_ruled_%s" % method)
                    updated = True
        if updated:
    ...
    def get_auditlog_fields(self, model):
        """
        Get the list of sicpro_modulo_auditorias fields for a models
    ...
    def _make_create(self):
        ...
        def create_full(self, vals_list, **kwargs):
            ...
            rule_model = self.env["sicpro.modulo.auditorias.rule"]
    ...
    def _make_read(self):
        ...
        def read(self, fields=None, load="_classic_read", **kwargs):
            ...
            # If the call came from sicpro_modulo_auditorias itself, skip logging:
            # avoid logs on `read` produced by sicpro_modulo_auditorias during internal
            # processing: read data of relevant records, 'ir.models',
            # 'ir.models.fields'... (no interest in logging such operations)
    ...
    def create_logs(
        ...
        """Create logs. `old_values` and `new_values` are dictionaries, Ej.:
        {RES_ID: {'FIELD': VALUE, ...}}
        """
        ...
        log_model = self.env["sicpro.modulo.auditorias.log"]
        http_request_model = self.env["sicpro.modulo.auditorias.http.request"]
        http_session_model = self.env["sicpro.modulo.auditorias.http.session"]
    ...
    def _get_field(self, model, field_name):
        ...
        if field_name not in cache.get(model.model, {}):
            ...
            # - search the field in the current models and those it inherits
    ...
    def _create_log_line_on_read(self, log, fields_list, read_values):
        ...
        log_line_model = self.env["sicpro.modulo.auditorias.log.line"]
    ...
    def subscribe(self):
        """Subscribe Rule for auditing changes on models and apply shortcut
        to view logs on that models.
        """
        ...
        for rule in self:
            ...
            vals = {
                "name": _("View logs"),
                "res_model": "sicpro.modulo.auditorias.log",
    ...
    def unsubscribe(self):
        """Unsubscribe Auditing Rule on models."""
