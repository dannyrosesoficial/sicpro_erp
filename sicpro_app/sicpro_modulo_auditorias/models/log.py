...
class AuditlogLog(models.Model):
    _name = "sicpro.modulo.auditorias.log"
    _description = "Auditorias - Log"
    ...
    line_ids = fields.One2many("sicpro.modulo.auditorias.log.line", "log_id", string="Fields updated")
    http_session_id = fields.Many2one("sicpro.modulo.auditorias.http.session", string="Session")
    http_request_id = fields.Many2one("sicpro.modulo.auditorias.http.request", string="HTTP Request")
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if not vals.get("model_id"):
                raise UserError(_("No models defined to create log."))
...
class AuditlogLogLine(models.Model):
    _name = "sicpro.modulo.auditorias.log.line"
    _description = "Auditorias - Log detalles (fields updated)"

    field_id = fields.Many2one(
        "ir.model.fields", ondelete="set null", string="Field", index=True
    )
    log_id = fields.Many2one(
        "sicpro.modulo.auditorias.log", string="Log", ondelete="cascade", index=True
    )
