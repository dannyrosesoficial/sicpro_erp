...
class ApiConector(models.Model):
    ...
    name = fields.Selection(selection_add=[('sicpro.app.transporte.sipetc', 'APLICACIÓN DE TRANSPORTE - SIPETC')],
                            ondelete={'sicpro.app.transporte.sipetc': 'cascade'})
    ...
    def conector_api_test_sicpro_app_transporte_sipetc(self):
        app = self.env['sicpro.modulo.api.conector'].sudo().search([('name', '=', 'sicpro.app.transporte.sipetc')])
        usuario = app.usuario
        ...
        if usuario and password:
            ...
            base64Credencial = base64.b64encode(data.encode("utf-8")).decode("utf-8")
            ...
            if response.status_code == 200:
                ...
                headerData = {'Authorization': 'Basic' + base64Credencial, 'X-AUTH-TOKEN': authTokenValue}
                # envío credenciales al url del data
                response = requests.get(url_data, headers=headerData, verify=False)
                if response.status_code == 200:
                ...
                raise ValidationError(_('Conexión reusada en el url Login, Verifíquelo'))
        else:
            raise ValidationError(_('Los campos de usuario o contraseña están vacíos, verifíquelo'))
        ...
        if app:
            ...
            if response.status_code == 200:
                ...
                if response.status_code == 200:
                    ...
                    todos_reg = self.env['sicpro.app.transporte.general'].sudo().search([('active', '=', True)])
                    ...
                    for item in equipos:
                        ...
                        data = self.env['sicpro.app.transporte.general'].sudo().search(
                        ...
                        jefe_id = self.env['sicpro.app.trabajadores'].search(
                            [('identification_id', '=', item['jefeChoferCi'])]).id

                        # Busco él id de la marca en los nomencladores del sicpro por el nombre de la marca según SIPETC
                        marca_id = self.env['sicpro.app.transporte.modelo'].search(
                            [('name', '=', item['marcaNombre'])]).id

                        if data.matricula:
                            # actualizo registros del transporte existente en sicpro desde el SIPE TC
                            data.sudo().write(
                                {'unidadNombre': item['unidadNombre'], 'unidadAcronimo': item['unidadAcronimo'],
                                 'areaIdentificacion': item['areaIdentificacion'], 'areaNombre': item['areaNombre'],
                                 'grupoEquipoNombre': item['grupoEquipoNombre'], 'tipoNombre': item['tipoNombre'],
                                 'marcaNombre': item['marcaNombre'], 'modeloNombre': item['modeloNombre'],
                                 'especialidadNombre': item['especialidadNombre'],
                                 'actividadNombre': item['actividadNombre'],
                                 'actividadFundamentalNombre': item['actividadFundamentalNombre'],
                                 'estadoTecnicoNombre': item['estadoTecnicoNombre'], 'color': item['color'],
                                 'combustibleNombre': item['combustibleNombre'], 'choferNombre': item['choferNombre'],
                                 'choferCi': item['choferCi'], 'choferCargo': item['choferCargo'],
                                 'choferDireccion': item['choferDireccion'], 'choferLicencia': item['choferLicencia'],
                                 'choferEsChoferProfesional': item['choferEsChoferProfesional'],
                                 'jefeChoferNombre': item['jefeChoferNombre'], 'jefeChoferCi': item['jefeChoferCi'],
                                 'jefeChoferCargo': item['jefeChoferCargo'],
                                 'jefeChoferDireccion': item['jefeChoferDireccion'],
                                 'jefeChoferLicencia': item['jefeChoferLicencia'],
                                 'jefeChoferEsChoferProfesional': item['jefeChoferEsChoferProfesional'],
                                 'parqueoNombre': item['parqueoNombre'], 'parqueoDireccion': item['parqueoDireccion'],
                                 'parqueoTipo': item['parqueoTipo'], 'parqueoProvincia': item['parqueoProvincia'],
                                 'capacidadCargaNombre': item['capacidadCargaNombre'], 'codigoEquipo': item['codigoEquipo'],
                                 'numeroCirculacion': item['numeroCirculacion'],
                                 'numeroInventario': item['numeroInventario'], 'fechaRecibo': item['fechaRecibo'],
                                 'annoFabricacion': item['annoFabricacion'], 'vin': item['vin'],
                                 'numeroSerie': item['numeroSerie'], 'numeroMotor': item['numeroMotor'],
                                 'marcaMotorNombre': item['marcaMotorNombre'],
                                 'modeloMotorNombre': item['modeloMotorNombre'],
                                 'indiceConsumoNormado': item['indiceConsumoNormado'],
                                 'indiceConsumoFabrica': item['indiceConsumoFabrica'],
                                 'tieneOdometro': item['tieneOdometro'], 'odometroOk': item['odometroOk'],
                                 'tieneHorametro': item['tieneHorametro'], 'horametroOk': item['horametroOk'],
                                 'esAlquilado': item['esAlquilado'], 'observacion': item['observacion'],
                                 'esParalizado': item['esParalizado'], 'estaDefectuoso': item['estaDefectuoso'],
                                 'estaActivo': item['estaActivo'],
                                 'name': item['choferNombre'] + ": " + item['matricula'] + " " + item['tipoNombre'] + " " +
                                         item['marcaNombre'] + " " + item['modeloNombre'],
                                 'fecha_actualizado': datetime.today(), 'chofer_trabajador_id': chofer_id,
                                 'jefe_trabajador_id': jefe_id, 'marca_id': marca_id, })
                            # actualizo la cantidad de registros actualizados
                            registros_actualizados += 1
                        else:
                            # creo registros del transporte por cada uno que existen en el SIPE TC
                            self.env['sicpro.app.transporte.general'].sudo().create(
                                {'unidadNombre': item['unidadNombre'], 'unidadAcronimo': item['unidadAcronimo'],
                                 'areaIdentificacion': item['areaIdentificacion'], 'areaNombre': item['areaNombre'],
                                 'grupoEquipoNombre': item['grupoEquipoNombre'], 'tipoNombre': item['tipoNombre'],
                                 'marcaNombre': item['marcaNombre'], 'modeloNombre': item['modeloNombre'],
                                 'matricula': item['matricula'], 'especialidadNombre': item['especialidadNombre'],
                                 'actividadNombre': item['actividadNombre'],
                                 'actividadFundamentalNombre': item['actividadFundamentalNombre'],
                                 'estadoTecnicoNombre': item['estadoTecnicoNombre'], 'color': item['color'],
                                 'combustibleNombre': item['combustibleNombre'], 'choferNombre': item['choferNombre'],
                ...
                for participante in seguidores:
                    ...
                    template = self.env.ref('sicpro_modulo_api_conector_transporte_sipetc.transporte_conector_rest_api_exito')
                ...
                for participante in seguidores:
                    ...
                    template.with_context(local_context).send_mail(app.id, force_send=True,
            ...
            print("No se ha configurado el servicio para la APLICACIÓN DE TRANSPORTE - app_id:siptc, "
