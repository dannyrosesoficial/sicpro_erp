...
LOG_FILE_PATH = "/opt/odoo/sicproerp/log/backup_cron.log"
# BACKUP_SCRIPT_PATH = "/odoo/webkul_addons/sicpro_modulo_backup_server/models/lib/saas_client_backup.py"
...
try:
    ...
    _logger.error("Módulo crontab no encontrado. Instálelo usando pip: pip3 install python-crontab")
...
class Cronjob:
    ...
    def create_command(self, masterpswd, url, main_db, db_name, db_user, db_password, process_id, backup_location, storage_path, module_path, backup_format="zip",kwargs = {}):
        _logger.info("===============CREANDO CMD DE COPIA DE SEGURIDAD=======================")
    ...
    # set_cron_for_new_client(masterpwd, url, nombre de base de datos, ubicación_de_copia de seguridad, intervalo, tiempo_de_creación, ruta_de_almacenamiento, ciclo)
def add_cron(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, frequency, frequency_cycle, storage_path, module_path, backup_format="zip", backup_starting_time=None, kwargs={}):
    ...
    wc = cj.write_crontab()
    if wc['success']:
        _logger.info("Trabajo agregado exitosamente")
    else:   
...
def update_cron(db_name, process_id, frequency, frequency_cycle):
    ...
    if uc['success'] == False:
        ...
        wc = cj.write_crontab()
        if wc['success']:
            _logger.info("Trabajo actualizado exitosamente")
        else:
            _logger.info("ERROR: %s"%(wc['msg']))
        return wc
...
def remove_cron(db_name, process_id, frequency, frequency_cycle):
    ...
    wc = cj.write_crontab()
    if wc['success']:
        _logger.info("Trabajo eliminado con éxito")
    else:
        _logger.info("ERROR: %s"%(wc['msg']))
...
if __name__ == '__main__':
    ...
    module_path = '/opt/odoo/sicproerp/sicpro_modulo_backup_server/models/lib/saas_client_backup.py'
    storage_path = os.getcwd()
    update_cron(db_name, frequency, frequency_cycle)
    # add_cron(contraseña_maestra, url, base_bd_principal, nombre_bd, usuario_bd, contraseña_bd, id_proceso, ubicación_copia de seguridad, frecuencia, ciclo_frecuencia, ruta_almacenamiento, ruta_módulo)
