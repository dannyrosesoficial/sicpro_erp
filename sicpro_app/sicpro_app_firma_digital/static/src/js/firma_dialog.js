/** @odoo-module alias=sicpro_app_firma_digital.firma_dialog **/

import core from 'web.core';
import Dialog from 'web.Dialog';
import {NameAndSignature} from 'sicpro_app_firma_digital.name_and_signature';
...
var SignatureDialog = Dialog.extend({
    template: 'firma_digital.firma_dialog',
    ...
    init: function (parent, options) {
        ...
        options.title = options.title || _t("Diseñar su firma manual");
        options.size = options.size || 'small';
        options.technical = false;

        if (!options.buttons) {
            options.buttons = [];
            options.buttons.push({text: _t("Aprobar y firmar"), classes: "btn-primary", disabled: true, click: function (e) {
                self._onConfirm();
            }});
            options.buttons.push({text: _t("Cancelar"), close: true});
        }
