...
class AppCMIReportes(models.TransientModel):
    ...
    def reporte_cmi_anios_ejes(self):
        ...
        ejes_est = []
        ejes_estrategicos = self.env[
            'sicpro.app.cmi.perspectivas.eje.estrategico'].search(
            [('active', '=', True)])

        for ejes in ejes_estrategicos:
            ejes_est.append({'id': ejes.id, 'name': ejes.name, })
        return ejes_est
    ...
    def reporte_cmi_anios_perspectivas(self):
        ...
        for pers in obj_perspectivas:
            obj_anuales_ids = pers.env[
                'sicpro.app.cmi.objetivos.anuales'].search(
            ...
            perspectivas.append({'id': pers.id, 'name': pers.name,
    ...
    def reporte_cmi_anios_estrategicos(self):
        ...
        for est in obj_estrategico_ids:
            ...
            estrategicos.append(
                {'id': est.id, 'perspectivas_id': est.perspectivas_id.id,
    ...
    def reporte_cmi_anios_anuales(self):
        ...
        for anu in obj_anuales_ids:
            ...
            anuales.append(
                {'id': anu.id, 'obj_estrategico_id': anu.obj_estrategico_id.id,
                    'name': anu.name,
    ...
    def reporte_cmi_anios_indicadores(self):
        ...
        for ind in obj_indicadores_ids:
            ...
            indicadores.append({'id': ind.id, 'anio': ind.anio,
                'obj_anuales_id': ind.obj_anuales_id.id, 'name': ind.name,
                'real_acumulado': real, 'meta_acumulado': meta,
                'porciento_avance_barra': porciento_avance_barra,
    ...
    def reporte_cmi_anios_acciones(self):
        ...
        for acc in acciones_ids:
            acciones.append({'id': acc.id, 'anio': acc.anio,
                'indicador_id': acc.indicador_id.id, 'name': acc.name,
                'fecha_inicio': acc.fecha_inicio, 'fecha_fin': acc.fecha_fin,
