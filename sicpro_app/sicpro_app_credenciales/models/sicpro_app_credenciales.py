...
class Credenciales(models.Model):
    ...
    def imagen_por_defecto(self):
        ...
        image_path = os.path.join(current_path, '..', 'static', 'src', 'img', 'default_image.png')
    ...
    telefono_trabajo = fields.Char(string='Teléfono Trabajo', related='trabajador.telefono_trabajo', store=True)
    movil_trabajo = fields.Char(string='Móvil Trabajo', related='trabajador.movil_trabajo', store=True)
    correo_trabajo = fields.Char(string='Correo Trabajo', related='trabajador.correo_trabajo', store=True)

    company_id = fields.Many2one('res.company', string='Proceso', related='trabajador.company_id', store=True)
    ocupacion_id = fields.Many2one('sicpro.app.trabajadores.ocupacion', string='Puesto de trabajo',
                                   related='trabajador.ocupacion_id', store=True)
    area_id = fields.Many2one('sicpro.app.trabajadores.areas', string='Departamento', related='trabajador.area_id', store=True)
    parent_id = fields.Many2one('sicpro.app.trabajadores', string='Jefe Inmediato', related='trabajador.parent_id', store=True)
    ...
    fecha_baja = fields.Date(string='Fecha Baja', related='trabajador.fecha_baja', store=True)
    ...
    cancelacion_active = fields.Boolean(string='Cancelacion_active', default=False, required=False, index=True)

    especial_telefono_trabajo = fields.Char(string='Teléfono Trabajo (Esp.)')
    especial_movil_trabajo = fields.Char(string='Móvil Trabajo (Esp.)')
    especial_correo_trabajo = fields.Char(string='Correo Trabajo (Esp.)')
    especial_company_id = fields.Char(string='Proceso (Esp.)')
    especial_ocupacion_id = fields.Char(string='Puesto de trabajo (Esp.)')
    especial_area_id = fields.Char(string='Departamento (Esp.)')
    ...
    especial_fecha_baja = fields.Date(string='Fecha Baja (Esp.)')
    ...
    def _check_unique_trabajador(self):
        for record in self:
            # Buscamos si existe otro registro con el mismo trabajador que no sea el actual
            domain = [('trabajador', '=', record.trabajador.id), ('id', '!=', record.id)]
            if self.search_count(domain) > 0:
                raise ValidationError("¡El nombre del trabajador existe!, verifíquelo.\n\n" + MSG_SOPORTE_SICPRO)
