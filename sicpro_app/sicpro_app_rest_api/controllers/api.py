...
def _get_model_obj(model, kwargs):
    ...
    # Usado en kwargs de verificación para forzar la eliminación
    # contexto cuando el valor {} está enviando
...
class RestApiDocs(Controller):
    @route("/api-docs/v<string:version>", auth='none', methods=["GET"])
    def api_docs(self, version=False, **kwargs):
        domain = [('name', '=', version)]
        version = request.env['api.rest.version'].search(domain, limit=1)
        if not version:
            return request.not_found()
        return request.render("sicpro_app_rest_api.openapi", {
            'url_swagger': version.url_swagger
        })

    @route("/api-docs/v<string:version>/swagger.json", auth='none',
           methods=["GET"])
    def api_json(self, version, **kwargs):
        ...
        version = request.env['api.rest.version'].search(domain, limit=1)
        if not version:
            return {'error': 'No se encontró la API'}
        data = version.get_swagger_json()
        return request.make_response(json.dumps(data), headers=[('Content-Type', 'application/json')])


class RestApi(Controller):
    @route('/api/v<string:_api_version>/<string:_api_name>', auth='none',
    ...
    def write(self, _api_version, _api_name, _api_id, _api_path, **kwargs):
        ...
        if not record:
            raise RecordNotFoundError(_('Registro no encontrado'))
