...
class ApiRestVersion(models.Model):
    ...
    _description = 'Registro de versiones del Api Rest'

    name = fields.Char(string='Número de la Versión', required=True)
    ...
    path_ids = fields.One2many(
        'api.rest.path', 'version_id', string='Directorio',
        context={'active_test': False})
    # Security
    user_ids = fields.Many2many('res.users', string='Usuarios')
    # Logs
    active_log = fields.Boolean(string='Activar Registros', )
    last_usage_date = fields.Datetime()
    log_ids = fields.One2many(
        'api.rest.log', 'version_id', string='Registros', readonly=True)
