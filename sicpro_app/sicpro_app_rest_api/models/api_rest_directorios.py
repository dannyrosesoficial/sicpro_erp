...
class ApiRestPath(models.Model):
    ...
    _order = 'model_id'
    _rec_name = 'model_id'
    _description = 'Registro de aplicaciones para el Api Rest'

    name = fields.Char(string='Nombre',required=True)
    active = fields.Boolean(string='Activo', default=True)
    version_id = fields.Many2one(
        'api.rest.version', string='Versión de la API', required=True,
        ondelete='cascade')
    model_id = fields.Many2one('ir.model', string='Modelo', required=True,
                               ondelete='cascade')
    model = fields.Char(string='Nombre del Modelo', related='model_id.model',
                        readonly=True)
    ...
    deprecated = fields.Boolean(string='Obsoleto')
    tag_id = fields.Many2one('api.rest.tag', string='Categoría', ondelete='set null')
    # leer
    filter_domain = fields.Char(string='Dominio', default="[]")
    field_ids = fields.Many2many(
        'ir.model.fields', domain="[('model_id', '=', model_id)]",
        string='Campos')
    limit = fields.Integer(string='Limite de resultados', default=1000)
    # crear / actualizar
    warning_required = fields.Boolean(
        compute='_compute_warning_required', compute_sudo=True)
    api_field_ids = fields.One2many(
        'api.rest.field', 'path_id', string='Campos.', copy=True)
    update_domain = fields.Char(string='Dominio para Actualizar', default="[]")
    # eliminar
    unlink_domain = fields.Char(string='Dominio para Eliminar', default="[]")
    # Custom function
    function_apply_on_record = fields.Boolean(
        string='Aplicar función personalizada')
    function_domain = fields.Char(string='Dominio de la función', default="[]")
    function = fields.Char(string='Función')
    function_parameter_ids = fields.One2many(
        'api.rest.function.parameter', 'path_id', string='Parámetros',
        copy=True)

    _sql_constraints = [
        ('name_uniq', 'unique (name, version_id, method)',
         "¡El nombre, la versión y el método deben ser únicos!"),
    ]
    ...
    def _generate_path(self, swagger_paths):
        ...
        # Post
            ...
            # Create element
            definition = {
                'description': _('Identificador del recurso creado.'),
                'schema': {
                    'type': 'integer',
                }
            }
        ...
        # Put
            ...
            # Update element
            definition = {
                'description': _('Devuelve un valor booleano si la actualización es un éxito.'),
                'schema': {
                    'type': 'boolean',
                }
            }
        ...
        # Delete
            ...
            # Delete element
            definition = {
                'description': _('Devuelve un valor booleano si la eliminación es un éxito.'),
                'schema': {
                    'type': 'boolean',
                }
            }
    ...
    def _generate_definition(self, swagger_definitions):
        ...
        swagger_definitions['ApiErrorResponse'] = {
            ...
            'properties': {
                'code': {
                    'type': 'integer',
                    'description': _('Código de error')
                },
                'error': {
                    'type': 'string',
                    'description': _('Nombre del error')
                },
                'description': {
                    'type': 'string',
                    'description': _('Descripción del error')
                }
            }
        }
        return True
    ...
    def _domain_parameter(self):
        ...
        return {
            ...
            'in': 'query',
            'description': '{} \n\n {}'.format(
                _('Buscar dominio para leer. (El valor predeterminado '
                  'es un dominio vacío que coincidirá con todos los registros)'),
                _('Ejemplo: `[(\'name\', \'=\', \'Test\')]`')),
            'required': False,
            'type': 'string',
        }
    ...
    def _offset_parameter(self):
        ...
        return {
            'name': 'offset',
            'in': 'query',
            'description': _('Número de registros a saltar. (Predeterminado a 0)'),
            'required': False,
            'type': 'integer',
        }
    ...
    def _limit_parameter(self):
        ...
        return {
            ...
            'default': self.limit,
            'description':
                _('Número máximo de registros a devolver. {}').format(
                    _('(Maximum: {})').format(
    ...
    def _order_parameter(self):
        ...
        return {
            ...
            'in': 'query',
            'description': '{} \n\n {}'.format(
                _('Param para ordenar el resultado. '
                  '(Predeterminado a ningún tipo) '
                  '\n Nombre de campo seguido del operador de clasificación. '
                  '(ascenso o descenso)'),
                _('Ejemplo: {}').format(
    ...
    def _context_parameter(self, type="query"):
        ...
        return {
            ...
            'in': type,
            'description': '{} \n\n {}'.format(
                _('Contexto específico del método'),
                _('Ejemplo: `{"lang": "es_ES"}`')),
            'type': 'string',
            'required': False,
        }
        ...
        if field.ttype == 'many2one':
            if definition:
                values.update({
                    ...
                    'description': '{} \n\n {}'.format(
                        _('Lista de retorno con 2 elementos, '
                          'primer ID del recurso (entero), '
                          'segundo nombre del recurso (cadena).'),
                        _('Example : `[1, \'Example\']`')),
                })
        # Manage dates
        if field.ttype == 'date':
            description = values.get('description', '')
            description += '\n\n {}'.format(_('Ejemplo: `YYYY-MM-DD`'))
            values.update({'description': description})
        if field.ttype == 'datetime':
            description = values.get('description', '')
            description += \
                '\n\n {}'.format(_('Ejemplo: `YYYY-MM-DD HH:MM:SS`'))
            values.update({'description': description})
        return values
