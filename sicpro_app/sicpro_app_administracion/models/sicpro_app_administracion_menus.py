    ...
    def _json_menus(self):
        ...
        for data in self:
            for obj in data.accesos:
                pre_action = str(obj.action.replace("'", ""))
                action = self.env.ref(pre_action).id

                dic.append(
                    {"id": obj.menu.id,
                     "menu_nombre": obj.name,
                     "menu_action": action,
                     })

            data.json_menus = json.dumps(dic)
            dic.clear()

    # ejecuta la acción específica solicitada
    def action_menu_admin(self):
        obj_menu = self._context.get('default_obj_menu')
...
class AdministracionAccesos(models.Model):
    ...
    # buscar la acción del menu
    @api.onchange('accesos')
    def _compute_action(self):
        for item in self:
            if item.accesos:
                item.action = str("'" + item.accesos.action.xml_id + "'")