...
class ViveresTrabajadoresEntregas(models.Model):
    ...
    jefe_area = fields.Many2one('sicpro.app.trabajadores',
                                string='Responsable',
                                related='name.name.manager_id', store=True, )
    active = fields.Boolean(string='Activo', default=True, index=True)
    trabajador_id = fields.Many2one('sicpro.app.trabajadores',
                                    string='Trabajador', )
    producto_comprado_trabajador = fields.Many2many(
    ...
    fecha_entrega = fields.Date(string='Fecha entrega al área',
                                compute='_compute_fecha_entrega_area',
                                readonly=True)
    fecha_entrega_trabajador = fields.Date(string='Fecha entrega al trabajador')
    ...
    def _compute_fecha_entrega_area(self):
        for each in self:
            entrega_area = self.env[
                ...
                ['&', ('name', '=', each.name.id), ('producto_comprado', 'in',
                                                    self.producto_comprado_trabajador.ids)])
    ...
    def restriccion_estado_pendiente(self):
        if self.estado == 'pendiente':
            if not self.observaciones:
                raise UserError(
                    'Debe especificar en las observaciones por qué queda pendiente la entrega al trabajador,' + MSG_SOPORTE_SICPRO)
    ...
    def restriccion_fecha_entrega_trabajador(self):
        if self.fecha_entrega_trabajador:
            ...
            if self.fecha_entrega_trabajador > today:
                raise UserError(
                    'La fecha de entrega al trabajador no puede ser superior al día de hoy,' + MSG_SOPORTE_SICPRO)
            elif self.fecha_entrega:
                if self.fecha_entrega_trabajador < self.fecha_entrega:
                    raise UserError(
                        'La fecha de entrega al trabajador no puede ser inferior a la fecha de entrega al área,' + MSG_SOPORTE_SICPRO)
