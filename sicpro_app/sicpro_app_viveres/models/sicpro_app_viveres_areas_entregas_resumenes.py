...
class ViveresAreasEntregasResumenes(models.Model):
    ...
    producto = fields.Many2one('sicpro.app.viveres.productos.comprados',
                               string='Producto', )
    fecha_compra = fields.Date(string='Fecha de compra', related='producto.fecha',
                               store=True)
    fecha_entrega = fields.Date(string='Fecha de entrega al área',
                                compute='_compute_fecha_entrega_area',
                                readonly=True)
    total_entregado_area = fields.Integer(string='Total entregado al área',
                                          compute='_compute_total_entregado_area',
                                          readonly=True)
    total_entregado_trabajadores = fields.Integer(string='Total de trabajdores beneficiados',
        compute='_compute_total_entregado_trabajadores_area', readonly=True)
    total_pendiente = fields.Integer(string='Total pendientes',
                                     compute='_compute_total_pendiente_trabajadores_area',
                                     readonly=True)
    total_no_entregado = fields.Integer(string='Total no entregados',
                                        compute='_compute_total_no_entregado_trabajadores_area',
                                        readonly=True)

    def _compute_fecha_entrega_area(self):
        for each in self:
            entrega_area = self.env[
    ...
    def _compute_total_entregado_trabajadores_area(self):
        for each in self:
            entrega_trabajador_area = self.env[
    ...
    def _compute_total_no_entregado_trabajadores_area(self):
        for each in self:
            entrega_area = self.env[
                'sicpro.app.viveres.areas.entregas'].search(
                ['&', ('producto_comprado', '=', each.producto.id),
                 ('name', '=', each.name.id)])
            if entrega_area:
