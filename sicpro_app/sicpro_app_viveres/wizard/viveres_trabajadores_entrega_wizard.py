...
class ViveresTrabajadoresEntregaWizard(models.TransientModel):
            ...
            if each:
                if each.area:
                    ...
                    if each.area.name.tipo_registro == 'sin_categoría':
                        trabajadores_area = self.env[
                            'sicpro.app.trabajadores'].search(
                            ['&', ('active', '=', True),
                             ('area_id.id', '=', each.area.name.id)])
                    else:
                        trabajadores_area = self.env[
                            'sicpro.app.trabajadores'].search(
                            ['&', ('active', '=', True), '|',
                    ...
                    if trabajadores_area and len(
                        self.productos_entregar_compute) > 0:
                        for trabajador in trabajadores_area:
                            entrega_trabajador = self.env[
                                'sicpro.app.viveres.trabajadores.entregas'].search(
                                ['&', ('producto_comprado_trabajador', 'in',
                                       self.productos_entregar.ids),
    ...
    productos_entregar_compute = fields.Many2many(
    ...
    productos_entregar = fields.Many2many(
        'sicpro.app.viveres.productos.comprados',
        'productos_entregar_trabajador_wizard_rel', 'productos_entregar_id',
        'wizard_id', string="Productos", )
    trabajadores_compute = fields.Many2many('sicpro.app.trabajadores',
                                            'trabajador_compute_wizard_rel',
                                            'trabajador_compute_id',
                                            'wizard_compute_trabajador_id',
                                            string='Entregado a Compute',
                                            compute='_compute_trabajadores_ids', )
    trabajadores = fields.Many2many('sicpro.app.trabajadores',
                                    'trabajador_wizard_rel', 'trabajador_id',
                                    'wizard_trabajador_id',
                                    string='Entregado a', )

    fecha_entrega = fields.Date(string='Fecha entrega', default=fields.Datetime.now())

    estado = fields.Selection(string="Estado", default='pendiente',
                              selection=[('entregado', 'Entregado'),
    ...
    def realizar_entrega(self):
        ...
        entregas_area = self.env['sicpro.app.viveres.areas.entregas'].search(
            ['&', ('active', '=', True), ('total_no_entregado', '>', 0),
             ('name', '=', self.area.id)])
        ...
        if not ok:
            raise UserError(
                "El listado de trabajadores es mayor a la cantidad de productos a entregar.\n\n" + MSG_SOPORTE_SICPRO)
        ...
        for trabajador in self.trabajadores:
            if trabajador:
                entrega_trabajador = self.env[
                    'sicpro.app.viveres.trabajadores.entregas'].search(['&', (
                'producto_comprado_trabajador', 'in',
                self.productos_entregar.ids), ('trabajador_id.id', '=',
                                               trabajador.id)])

                if not entrega_trabajador:
                    self.env[
                        'sicpro.app.viveres.trabajadores.entregas'].sudo().create(
                        {'name': self.area.id, 'trabajador_id': trabajador.id,
                         'producto_comprado_trabajador': self.productos_entregar.ids,
                         'fecha_entrega_trabajador': self.fecha_entrega,
                         'estado': self.estado})
                    total_entregado += 1

        entrega_area = self.env['sicpro.app.viveres.areas.entregas'].search(
            ['&', ('producto_comprado', 'in', self.productos_entregar.ids),
             ('name', '=', self.area.id)])
        if entrega_area:
        ...
        if self.estado:
            if self.estado == 'pendiente' or self.estado == 'no_entregado':
                if not self.observaciones:
                    raise UserError(
                        "Debe especificar en las observaciones el por qué no se entregó el producto.\n\n" + MSG_SOPORTE_SICPRO)
