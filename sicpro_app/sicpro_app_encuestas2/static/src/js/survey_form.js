...
var SurveyForm = publicWidget.Widget.extend({
    ...
    _onSelectCountry: function(ev){
        var self = this;
        rpc.query({route: '/survey/load_country', params: {}}).then(function (result){
            self.$el.find('#' + ev.target.id).html('<option value="">Country</option>')
            result['name'].forEach(function(name){ self.$el.find('#' + ev.target.id).append('<option value="' + name + '">' + name + '</option>'); });
        });
    },
    _onSelectState: function(ev){
        ...
        rpc.query({route: '/survey/load_states', params: { country_id }}).then(function (result){
            self.$el.find('#' + question_id + '-state').html('<option value="">State</option>')
            result['name'].forEach(function(name){ self.$el.find('#' + question_id + '-state').append('<option value="' + name + '">' + name + '</option>'); });
        });
    },
