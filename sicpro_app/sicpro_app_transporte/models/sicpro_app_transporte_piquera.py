...
class TransportePiquera(models.Model):
    ...
    name = fields.Char(string="Servicio", default="Solicitud de Piquera",
                       readonly=True, copy=False)
    solicitante = fields.Many2one('res.users', required=True,
                                  string='Solicitante')
    cargo = fields.Many2one('sicpro.app.trabajadores.ocupacion',
                            string='Cargo', store=True, copy=False,
                            related="solicitante.trabajador.ocupacion_id")
    movil = fields.Char(string="Teléfono", store=True, copy=False,
                        related='solicitante.trabajador.movil_trabajo')
    departamento = fields.Many2one('sicpro.app.trabajadores.areas', copy=False,
                                   string="Departamento", store=True,
                                   related="solicitante.trabajador.area_id")
    proceso = fields.Many2one('res.company', string="Proceso", store=True,
                              copy=False,
    ...
    solicitado = fields.Many2one('res.users', string='Elaborado por:',
                                 default=lambda self: self.env.uid,
                                 tracking=True)
    salida = fields.Many2one('sicpro.app.transporte.distancia',
                             string='Salida', tracking=True, required=True)
    destino = fields.Text(string="Destino", required=True)
    cantidad_personas = fields.Integer(string='No. de Personas', required=True)
    rechaza = fields.Many2one('res.users', string='Rechaza o Cancela',
                              tracking=True)
    cargo_rechaza = fields.Many2one('sicpro.app.trabajadores.ocupacion',
                                    string='Ocupación', store=True, copy=False,
                                    related="rechaza.trabajador.ocupacion_id")
    movil_rechaza = fields.Char(string="Móvil", store=True, copy=False,
                                related='rechaza.trabajador.movil_trabajo')
    transporte_id = fields.Many2one(
        comodel_name='sicpro.app.transporte.general', string='Vehiculo',
        domain="[('piquera', '=', True)]")
    chapa = fields.Char(string="Chapa", store=True, copy=False,
                        related='transporte_id.matricula')
    chofer = fields.Char(string="Chofer", store=True, copy=False,
                         related='transporte_id.choferNombre')
    ...
    color = fields.Char(string="Color", related='transporte_id.color',
                        store=True, copy=False)
    marca = fields.Char(string="Marca", store=True, copy=False,
                        related='transporte_id.marcaNombre')
    modelo = fields.Char(string="Modelo", store=True, copy=False,
                         related='transporte_id.modeloNombre')
    indice_consumo = fields.Float(string="Indice Consumo",
    ...
    estado = fields.Selection(
        [('borrador', 'Borrador'), ('pendiente', 'Pendiente'),
         ...
         ('aprobado', 'Aprobado')], string="Estado", default="borrador",
    ...
    cancelado = fields.Boolean(string='Cancelado', default=False,
                               required=False)
    desfasado = fields.Boolean(string='Desfasado', default=False,
                               required=False)
    recorridos_ids = fields.One2many(
        'sicpro.app.transporte.piquera.recorridos', 'name',
        string="Recorridos")
    combustible = fields.Many2one(
        comodel_name='sicpro.app.transporte.tipo.combustible',
        string='Combustible', required=False)
    company_id = fields.Many2one('res.company', string='Proceso solicitante',
                                 required=True,
                                 default=lambda self: self.env.company)
    company_currency = fields.Many2one(string='Moneda', readonly=True,
                                       related='company_id.currency_id')
    total_km = fields.Float(string="Total (Kilómetros)", readonly=True,
                            copy=False)
    valor_combustible = fields.Monetary(string="Valor del Combustible",
                                        related='combustible.costo',
                                        currency_field='company_currency',
                                        store=True, copy=False)
    total_litros = fields.Float(string="Consumo Total (L)", copy=False)
    total_costo = fields.Monetary(string="Total", readonly=True, copy=False,
                                  currency_field='company_currency')
    ...
    def total_updater(self):
        ...
        if self.transporte_id:
            ...
            raise ValidationError(
                "Debe seleccionar un vehículo para continuar\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def _compute_grupo_ejecutor(self):
        is_ejecutor = self.env.user.has_group(
            'sicpro_app_transporte.grupo_app_transporte_piquera_ejecutor')
        for record in self:
    ...
    def imprimir_costos(self):
        return {'type': 'ir.actions.report',
                'model': 'sicpro.app.transporte.piquera',
    ...
    def solicitar_piquera(self):
        ...
        # busco en el grupo de ejecutores de la piquera
        ejecutores = self.env['res.users']
        ejecutores_notif = self.env.ref(
            'sicpro_app_transporte.grupo_app_transporte_piquera_ejecutor',
            raise_if_not_found=False)
        if ejecutores_notif:
            ejecutores = ejecutores_notif.user_ids
        ...
        self.message_post(body='Solicitud de piquera',
        ...
        for participante in self.message_partner_ids:
            ...
            template = self.env.ref(
                'sicpro_app_transporte.transporte_solicitud_piquera')
            template.with_context(local_context).send_mail(self.id,
                                                           force_send=True,
                                                           email_values=email_values)
        # redirecciono la salida
        action = self.sudo().env.ref(
            'sicpro_app_transporte.transporte_piquera_action').read()[0]
        return action
    ...
    def aprobar_solicitud(self):
        if self.transporte_id and self.combustible:
            ...
            # envío la notificación a los seguidores
            self.message_post(body='Solicitud de piquera aprobada',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
            # Selecciono el registro de seguidores
            for participante in self.message_partner_ids:
                ...
                if self.hora_modificada:
                    template = self.env.ref(
                        'sicpro_app_transporte.transporte_aprobacion_mod_hora_piquera')
                else:
                    template = self.env.ref(
                        'sicpro_app_transporte.transporte_aprobacion_piquera')
                template.with_context(local_context).send_mail(self.id,
                                                               force_send=True,
                                                               email_values=email_values)
            # redirecciono la salida
            ...
            raise ValidationError(
                "Debe seleccionar un vehículo o tipo de combustible\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def _check_cantidad_personas(self):
        for item in self:
            if item.cantidad_personas == 0:
                raise ValidationError(
                    "Debe seleccionar la cantidad de personas que utilizaran el vehículo.\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def _check_hora_salida_regreso(self):
        for item in self:
            if item.hora_salida == 0 or item.hora_regreso == 0:
                raise ValidationError(
                    "Debe seleccionar la hora de salida y regreso "
                    "del servicio qué está solicitando.\n\n" + MSG_SOPORTE_SICPRO)

    @api.model_create_multi
    def create(self, vals_list):
        records = super(TransportePiquera, self).create(vals_list)
        for res in records:
            ...
            res.sequence_consecutivo = self.env['ir.sequence'].next_by_code(
                'piquera_consecutivo_incrementar')
            return res
        return None
...
class TransportePiqueraRecorridos(models.Model):
    ...
    salida = fields.Many2one(comodel_name='sicpro.app.transporte.distancia',
    ...
    recorrido = fields.Many2one(
        comodel_name='sicpro.app.transporte.distancia.recorrido',
        domain="['&', ('salida_id', '=', salida), "
               "('destino_id', '=', destino)]", string='Recorrido',
        required=True)
    km = fields.Float(string='Distancia(KM)', related="recorrido.km",
                      store=True)
    ...
    def _onchange_salida_destino(self):
        for item in self:
            if item.salida and item.destino:
                recorrido = self.env[
                    'sicpro.app.transporte.distancia.recorrido'].search(
                    ['&', ('salida_id', '=', self.salida.id),
                ...
                if item.salida == item.destino:
                    ...
                    raise ValidationError(
                        "La Salida y el Destino no tienen un recorrido creado.\n\n" + MSG_SOPORTE_SICPRO)
                else:
                    item.recorrido = None
...
class TransportePiqueraRechazadas(models.TransientModel):
    ...
    def piquera_motivo_rechazo(self):
        rechazo = self.env['sicpro.app.transporte.piquera'].browse(
        ...
        post = self.env['sicpro.app.transporte.piquera'].browse(
            self.env.context.get('active_ids'))
        post.message_post(body='Solicitud de piquera rechazada.',
        ...
        for participante in post.message_partner_ids:
            ...
            template = self.env.ref(
                'sicpro_app_transporte.transporte_rechazo_piquera')
            template.send_mail(post.id, force_send=True,
                               email_values=email_values)
        # redirecciono la salida
        action = self.env.ref(
            'sicpro_app_transporte.transporte_piquera_action').sudo().read()[0]
        return action
...
class TransportePiqueraCanceladas(models.TransientModel):
    ...
    def piquera_motivo_cancelacion(self):
        cancelacion = self.env['sicpro.app.transporte.piquera'].browse(
            self.env.context.get('active_ids'))
        ...
        post = self.env['sicpro.app.transporte.piquera'].browse(
            self.env.context.get('active_ids'))
        post.message_post(body='Solicitud de piquera cancelada.',
                          message_type='notification',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
        for participante in post.message_partner_ids:
            ...
            local_context = post.env.context.copy()
            template = self.env.ref(
                'sicpro_app_transporte.transporte_cancelacion_piquera')
            template.send_mail(post.id, force_send=True,
                               email_values=email_values)
        # redirecciono la salida
        action = self.env.ref(
