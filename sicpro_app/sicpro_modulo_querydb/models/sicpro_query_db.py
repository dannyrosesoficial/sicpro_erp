...
class QueryDB(models.Model):
    _name = "sicpro.querydb"
    _description = "Ejecuta consultas a Pstgresql desde SICPRO ERP"
    _inherit = ['mail.thread', 'mail.activity.mixin']

    tips = fields.Many2one('tipsqueries', string="Consultas")
    ...
    show_raw_output = fields.Boolean(string='Visualizar salida de la consulta')
    raw_output = fields.Text(string='Salida')

    def print_result(self):
        return {
            'name': _("Seleccione la orientación del PDF"),
            'view_mode': 'form',
...
class TipsQueries(models.Model):
    ...
    name = fields.Char(string='Consulta', required=True)
