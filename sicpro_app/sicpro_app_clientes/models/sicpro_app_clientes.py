...
class AppClientes(models.Model):
    ...
    user_id = fields.Many2one('res.users', 'Usuario', store=True, readonly=False)
    ...
    def _onchange_firma_acuerdo_servicio(self):
        if self.firma_acuerdo_servicio:
            jefe = self.env['sicpro.app.clientes'].search([('id', '=', self.entidad.id)])
    ...
    def _onchange_territorio_values(self, territorio):
        if territorio:
            cliente = self.env['sicpro.nomenclador.territorios'].browse(territorio)
