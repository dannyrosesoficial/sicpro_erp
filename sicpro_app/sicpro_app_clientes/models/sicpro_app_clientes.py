...
class AppClientes(models.Model):
    ...
    tipo_registro = fields.Selection(string="",
    ...
    nivel_escolar = fields.Selection(
        [('primaria', 'Primaria'), ('secundaria', 'Secundaria Básica'),
         ...
         ('superior', 'Superior'), ], 'Nivel Escolar', tracking=True)
    ...
    def _onchange_firma_acuerdo_servicio(self):
        if self.firma_acuerdo_servicio:
            ...
            if jefe.jefe_entidad:
                ...
                raise UserError(
                    'ERROR: La entidad ya tiene un jefe configurado.')
            else:
            ...
            self.env['sicpro.app.clientes'].search(
                [('id', '=', self.entidad.id)]).update({'jefe_entidad': False})
    ...
    def _onchange_user_id(self, ):
        if self.user_id:
            self.env['res.users'].search(
                [('id', '=', self.id_user_cliente)]).update(
                {'user_inversionista': False})
            self.env['res.users'].search(
                [('id', '=', self.user_id.id)]).update(
                {'user_inversionista': True})
    ...
    # devuelve nuevos valores a provincia cuando la
    # Unidad Organizativa ha cambiado
    def _onchange_territorio_values(self, territorio):
        if territorio:
            cliente = self.env['sicpro.nomenclador.territorios'].browse(
                territorio)
            return {'provincias_id': cliente.provincias_id}
        return {}

    # Al cambiar campo Unidad Organizativa
    ...
    def _onchange_entidad(self):
        ...
        self.provincias_id = self.entidad.provincias_id
