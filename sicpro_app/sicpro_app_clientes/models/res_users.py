...
class Users(models.Model):
    ...
    nombre_inversionista = fields.Many2one('sicpro.app.clientes', string='Nombre',
        domain="['&', ('es_entidad', '=', False), ('user_id', '=', False)]")
    nombre_inversionista_2 = fields.Many2one('sicpro.app.clientes', string='nombre_inversionista_2')
    inversionista_jefe_entidad = fields.Many2one(comodel_name='sicpro.app.clientes', string='Jefe Entidad', store=True,
                                                 related='nombre_inversionista.jefe_entidad')
    inversionista_etiquetas = fields.Many2many(comodel_name="sicpro.app.clientes.etiquetas", string="Etiquetas",
    ...
    inversionista_provincia = fields.Many2one(comodel_name="res.country.state", string="Provincia", store=True,
    ...
    inversionista_nivel_escolar = fields.Selection(string='Nivel Escolar Inversionista',
    ...
    def _onchange_nombre_inversionista(self, ):
        ...
        if self.nombre_inversionista:
            ...
            data = self.env['sicpro.app.clientes'].search([('id', '=', self.nombre_inversionista_2.id)])
            for item in data:
                item.user_id = None
                item.pin = None
            ...
            data = self.env['sicpro.app.clientes'].search([('id', '=', self.nombre_inversionista.id)])
            for item in data:
