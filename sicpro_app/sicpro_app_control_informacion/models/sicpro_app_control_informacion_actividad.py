...
class ControlInformacionActividades(models.Model):
    ...
    areas = fields.Many2many('sicpro.app.control.informacion.areas',
                             'areas_control_informacion_rel', string='Áreas',
                             required=True)
    dia_entrega = fields.Integer(string='Día de entregas', required=True, default=0)
    notificar = fields.Many2many('sicpro.app.control.informacion.dias',
                                 'notificar_control_informacion_rel',
                                 'actividades_id', 'dias_id',
                                 string='Notificaciones(días)', required=False)
    cuenta_mes_actual = fields.Integer(string="Cantidad del mes actual",
                                       compute='_compute_todo_control')
    cuenta_enviados = fields.Integer(string="Cantidad enviados",
                                     compute='_compute_todo_control')
    cuenta_pendientes = fields.Integer(string="Cantidad pendiente",
                                       compute='_compute_todo_control')
    cuenta_validados = fields.Integer(string="Cantidad validada",
                                      compute='_compute_todo_control')
    cuenta_atrasados = fields.Integer(string="Cantidad de atrasado",
                                      compute='_compute_todo_control')
    ...
    def _compute_todo_control(self):
        ...
        nombre_mes = self.env['sicpro.nomenclador.meses'].search(
            ['&', ('active', '=', True),
             ('codigo_mes', '=', datetime.today().month)])
        ...
        for item in self:
            if item.dia_entrega < 1 or item.dia_entrega > 30:
                raise ValidationError(
                    "Los días de entrega no puede ser anterior a 1 o superior a 30.\n\n" + MSG_SOPORTE_SICPRO)
