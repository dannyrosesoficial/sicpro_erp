...
class Trabajos(models.Model):
    ...
    name = fields.Many2one(
        comodel_name='sicpro.app.trabajadores.cargos',
    ...
    _sql_constraints = [
        ('name_uniq', 'unique(name, company_id, area_id)',
         'Ya existe ese puesto de trabajo para el departamento '
    ...
    def _compute_cuenta_trabajadores(self):
        trabajadores_data = self.env['sicpro.app.trabajadores'].read_group(
            [('ocupacion_id', 'in', self.ids)], ['ocupacion_id'], ['ocupacion_id'])
        result = dict((data['ocupacion_id'][0], data['ocupacion_id_count'])
