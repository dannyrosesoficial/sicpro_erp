...
class TrabajadoresGeneral(models.Model):
    ...
    trabajador_id = fields.Many2one(
        comodel_name='sicpro.app.trabajadores', string='Trabajador_id')
    ...
    passport_attachment_id = fields.Many2many(
        'ir.attachment', 'passport_attachment_rel',
    ...
    visa_expira = fields.Date('Expiración visado', tracking=True)
    ...
    donante = fields.Selection(
        string='Donante', selection=[('si', 'Es Donante'),
                                     ('no', 'No es Donante'), ],
    ...
    salario_extra = fields.Float("Salario extra", store=True,
    ...
    nexo_familiar_ids = fields.One2many(
        'sicpro.app.trabajadores.nexo.familiar',
        'trabajadores_id', string='Nexo Familiar',)
    ...
    resume_line_ids = fields.One2many(
        'sicpro.app.trabajadores.educacion', 'employee_id',
        string=" Registro educativo")
    employee_skill_ids = fields.One2many(
        'sicpro.app.trabajadores.cursos', 'employee_id', string=" Cursos")

    job_title = fields.Char("Job Title", compute="_compute_job_title",
                            store=True, readonly=False)
    subordinate_ids = fields.One2many(
        'sicpro.app.trabajadores', string='Subordinados',
        compute='_compute_subordinates', compute_sudo=True)
    child_all_count = fields.Integer(
        string='Subordinados Indirectos Count',
    ...
    equipo_tecnico = fields.Many2one(
    ...
    grupo_seguridad_salud = fields.Boolean(
        string='grupo_seguridad_salud',
    ...
    seguridad_ids = fields.One2many(
        'sicpro.app.trabajadores.seguridad.trabajador',
        'trabajador_id', string="Seguridad y Protección")
    seguridad_ocupacion = fields.Integer(string='SO',
                                         compute='compute_seguridad_ocupacion')

    # genera el id del cargo del trabajador para utilizarlo
    # en los medios de protección
    ...
    # grupo Responsable o al grupo Capacitador
    def _compute_grupo_capacitador(self):
        responsable = self.env['res.users'].has_group(
    ...
    # verifica q el usuario activo pertenezca al
    # grupo Responsable o al grupo Seguridad y Salud
    ...
    def _compute_disciplinaria_count(self):
        all_actions = self.env['sicpro.app.trabajadores.disiplinaria.acciones'].read_group(
            [('trabajador_id', 'in', self.ids), ('estado', '=', 'action'), ],
            fields=['trabajador_id'], groupby=['trabajador_id'])
        mapping = dict(
            [(action['trabajador_id'][0], action['trabajador_id_count']) for
             action in all_actions])
        for trabajador in self:
            trabajador.disciplina_count = mapping.get(trabajador.id, 0)

    # Abre la vista de los documentos de los trabajadores
    ...
    def disciplina_view(self):
        ...
        return {'name': _('Medidas Disciplinarias'), 'domain': domain,
                ...
                'help': _('''<p class="oe_view_nocontent_create">
                               Click para crear una nueva medida disciplinaria
                            </p>'''), 'limit': 80,
    ...
    def documentos_view(self):
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'help': _('''<p class="oe_view_nocontent_create">
                           Click para crear un nuevo documento
                        </p>'''),
            'limit': 80,
    ...
    # establece el estado de conexion del trabajador
    @api.depends('user_id.im_status')
    def _compute_presence_state(self):
        # Chequeo el login
        for data in self:
            estado = 'to_define'
            if data.user_id.im_status == 'online' or data.last_activity:
                estado = 'present'
            elif data.user_id.im_status == 'offline' and data.id:
                estado = 'absent'
            data.estado_conexion = estado

    # establece el estado de la ultima conexion del trabajador
    @api.depends('user_id')
    def _compute_last_activity(self):
        estado = self.env['bus.presence'].search_read(
            [('user_id', 'in', self.mapped('user_id').ids)],
            ['user_id', 'last_presence'])
        estado = {p['user_id'][0]: p['last_presence'] for p in estado}

        for trabajador in self:
            tz = trabajador.tz
            last_presence = estado.get(trabajador.user_id.id, False)
            if last_presence:
                last_activity_datetime = last_presence.replace(
                    tzinfo=UTC).astimezone(timezone(tz)).replace(tzinfo=None)
                trabajador.last_activity = last_activity_datetime.date()
                if trabajador.last_activity == fields.Date.context_today(self):
                    trabajador.last_activity_time = format_time(self.env,
                                                                last_activity_datetime,
                                                                time_format='short')
                else:
                    trabajador.last_activity_time = False
            else:
                trabajador.last_activity = False
                trabajador.last_activity_time = False
    ...
    # envío felicitación de cumpleaños via correo
