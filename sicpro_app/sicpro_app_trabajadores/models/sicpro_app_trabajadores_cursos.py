...
class TrabajadoresCursos(models.Model):
    ...
    skill_level_id = fields.Many2one('sicpro.app.trabajadores.cursos.niveles',
                                     string='Nivel de Habilidad', required=True)
    ...
    def _check_skill_type(self):
        for record in self:
            if record.skill_id not in record.skill_type_id.skill_ids:
                raise ValidationError(_("La habilidad %(name)s y tipo de habilidad %(type)s no coincide",
    ...
    def _check_skill_level(self):
        for record in self:
            if record.skill_level_id not in record.skill_type_id.skill_level_ids:
                raise ValidationError(_("El nivel de habilidad %(level)s no es válido para el tipo de habilidad: %(type)s",
