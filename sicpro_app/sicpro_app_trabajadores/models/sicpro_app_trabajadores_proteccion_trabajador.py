...
class TrabajadoresProteccionTrabajador(models.Model):
    ...
    name = fields.Many2one(comodel_name='sicpro.app.trabajadores.seguridad.proteccion', string='Descripción',
                           required=False, domain="[('cargo', '=', cargo)]")
    talla = fields.Many2one('sicpro.app.trabajadores.tallas', string='Talla', required=False)
    ...
    vida_util = fields.Char(string='Vida Util', related='name.vida_util', store=True)
    unida_medida = fields.Char(string='Unidad de Medidas', store=True, related='name.unida_medida')
    cargo_get = fields.Integer(string='Cargo_get', required=False)
    cargo = fields.Many2one('sicpro.app.trabajadores.cargos', string="Cargo Asociado", required=True,
    ...
    anio = fields.Many2one(comodel_name='sicpro.nomenclador.anios', string='Año', required=True)
    company_id = fields.Many2one('res.company', string='Proceso', store=True, related='trabajador_id.company_id')
    departamento = fields.Many2one('sicpro.app.trabajadores.areas', string="Area", required=False, store=True,
    ...
    precio = fields.Monetary("Precio", currency_field='company_currency_id', related='name.precio', store=True)
    ...
    def _onchange_entregado(self):
        if self.demanda < self.entregado:
            self.demanda = None
            raise UserError(_('El valor de la entrega supera a la demanda. Verifíquelo.'))
