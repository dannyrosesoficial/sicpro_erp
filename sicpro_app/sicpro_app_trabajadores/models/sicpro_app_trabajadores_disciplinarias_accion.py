...
class TrabajadoresAccionesDisciplinarias(models.Model):
    ...
    estado = fields.Selection(
        [('borrador', 'Borrador'), ('espera_accion', 'Esperando acción'), ('action', 'Acción validada'),
         ('cancelado', 'Cancelado'), ], default='borrador', tracking=True)
    ...
    name = fields.Char(string='Referencia', required=False, copy=False, readonly=False, )
    user_id = fields.Many2one('res.users', index=True, tracking=True, default=lambda self: self.env.uid)
    solicita_medida = fields.Many2one('sicpro.app.trabajadores', string='Solicitante', required=True,
                                      default=_default_solicitante)
    cargo_solicitante = fields.Many2one('sicpro.app.trabajadores.ocupacion', string='Cargo', store=True,
                                        related='solicita_medida.ocupacion_id')
    departamento_solicitante = fields.Many2one('sicpro.app.trabajadores.areas', string='Departamento', store=True,
                                               related='solicita_medida.area_id')
    company_id = fields.Many2one('res.company', store=True, readonly=False, related='trabajador_id.company_id')
    trabajador_id = fields.Many2one('sicpro.app.trabajadores', string='Trabajador', required=True, )
    departamento = fields.Many2one('sicpro.app.trabajadores.areas', string='Área', related='trabajador_id.area_id',
                                   store=True)
    fecha_incorporacion = fields.Date(string="Incorporación", related='trabajador_id.fecha_incorporacion', store=True)
    ...
    motivo = fields.Many2one('sicpro.app.trabajadores.disiplinaria.categorias', string='Motivo', required=True)
    action = fields.Many2one('sicpro.app.trabajadores.disiplinaria.categorias', string="Medida")
    dias_validacion = fields.Integer(string='Días', required=False, related='action.vigencia', store=True)
    ...
    fecha_solicitud = fields.Date(string="Medida solicitada", default=lambda self: fields.Date.context_today(self))
    fecha_emision = fields.Date(string="Medida emitida", compute='_compute_action')
    fecha_notificacion = fields.Date(string="Trabajador notificado", compute='_compute_action')
    fecha_expiracion = fields.Datetime(string='Fecha de Expiración', readonly=False)
    active = fields.Boolean('Activo', default=True)
    estado_secundario = fields.Char(string='estado_secundario', default='Borrador')
    motivo_description = fields.Text(string="Detalles del motivo", related='motivo.description')
    accion_description = fields.Text(string="Detalles la acción", related='action.description')
    ...
    cargo = fields.Many2one('sicpro.app.trabajadores.ocupacion', string='Cargo:', store=True,
                            related='trabajador_id.ocupacion_id')
    fecha_contrato = fields.Date(string="Contratación", store=True, related='trabajador_id.inicio_contrato')
    categoria_ocupacional = fields.Many2one('sicpro.app.trabajadores.categorias', string='Categoría', store=True,
    ...
    def assign_function(self):
        ...
        self.message_post(body='Medida en espera de revisión', subtype_xmlid='mail.mt_comment',
    ...
    def cancel_function(self):
        # envió la notificación a los seguidores
        self.message_post(body='Medida cancelada', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
    ...
    def set_to_function(self):
        # envió la notificación a los seguidores
        self.message_post(body='Medida en estado de borrador', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
    ...
    def action_function(self):
        for rec in self:
            if not rec.action:
                raise ValidationError(_('¡Debes seleccionar una acción!'))
            if not rec.acciones_detalles:
                raise ValidationError(_('¡Debes que completar la información de la acción!'))
            # envió la notificación a los seguidores
            self.message_post(body='Medida validada', subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
    ...
    def send_expira_medida_mail(self):
        ...
        trabajador = self.env['sicpro.app.trabajadores.disiplinaria.acciones'].search(
            ['&', ('active', '=', True), ('fecha_expiracion', '=', expira)])
        if trabajador:
            for emp in trabajador:
                # envió la notificación a los seguidores
                emp.message_post(body='Medida disciplinaria expirada', message_type='notification',
                ...
                for participante in emp.message_partner_ids:
                    ...
                    participantes = participante.email_formatted
                    email_values = {'email_to': participantes}
                    template = self.env.ref('sicpro_app_trabajadores.trabajadores_medidas_disciplinarias_expira')
                    template.send_mail(emp.id, force_send=True, email_values=email_values, )

    @api.model
    def create(self, vals):
        res = super(TrabajadoresAccionesDisciplinarias, self).create(vals)
        # asignar la secuencia para el registro
        res['name'] = self.env['ir.sequence'].next_by_code('medidas_disciplinarias')
        ...
        legal = self.env.ref('sicpro_app_trabajadores.grupo_app_trabajador_disciplina_legal').users
        # busco los representantes del proceso del trabajador afectado
        representantes_proceso = self.env.ref('sicpro_app_trabajadores.grupo_app_trabajador_disciplina_jefe').users.ids
        representantes = self.env['res.users'].search(
            ['&', ('company_id', '=', trabajador.company_id.id), ('id', 'in', representantes_proceso)])
        ...
        # envió la notificación a los seguidores
        res.message_post(body='Medida disciplinaria en borrador', subtype_xmlid='mail.mt_comment',
                         author_id=self.env.user.partner_id.id)
        ...
        for participante in seguidores:
            ...
            participantes = participante.email_formatted
            email_values = {'email_to': participantes}
            template = self.env.ref('sicpro_app_trabajadores.trabajadores_medidas_disciplinarias_nueva')
