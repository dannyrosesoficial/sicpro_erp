...
class Channel(models.Model):
    ...
    subscription_department_ids = fields.Many2many(
        'sicpro.app.trabajadores.departmentos', string='Departamentos',
    ...
    def _subscribe_users(self):
        ...
        for mail_channel in self:
            if mail_channel.subscription_department_ids:
                mail_channel.write(
                    {'channel_partner_ids':
                        [(4, partner_id) for partner_id in mail_channel.mapped(
                            'subscription_department_ids.member_ids.user_id.partner_id').ids]})
