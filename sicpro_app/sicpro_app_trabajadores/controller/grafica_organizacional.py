...
class EmployeeChart(http.Controller):
    ...
    @http.route('/get/parent/colspan', type='json', auth='public', csrf=False)
    def get_col_span(self, emp_id):
        if emp_id:
            employee = request.env['sicpro.app.trabajadores'].sudo().browse(int(emp_id))
            if employee.child_ids:
                child_count = len(employee.child_ids) * 2
                return child_count

    @http.route('/get/parent/employee', type='json', auth='public', csrf=False)
    def get_employee_ids(self):
        employees = request.env['sicpro.app.trabajadores'].sudo().search([('parent_id', '=', False)])
        names = []
        key = []
        if len(employees) == 1:
            ...
            raise UserError("En el organigrama el director no debe tener jefe inmediato superior.")
        else:
            for emp in employees:
                names.append(emp.name)
            raise UserError("Estos trabajadores no tienen jefe inmediato superior %s" % (names))
        ...
        if child_ids:
            ...
            for child in child_ids:
                ...
                view = """ <div id='""" + str(child.id) + """' class='o_level_1'><a>
                    ...
                    <img src='/web/image/sicpro.app.trabajadores/""" + str(child.id) + """/image_1024/'/></div>
                    <div class='employee_name'><p>""" + str(child.name) + """</p>
                    <p>""" + str(child.ocupacion_id.name.name) + """</p></div></a></div>"""
                child_nodes += child_table + view + """</div></td></tr></table></td>"""
            nodes = child_nodes + """</tr>"""
            return nodes

    @http.route('/get/parent/child', type='http', auth='user', csrf=False)
    def get_parent_child(self, **post):
        if post:
            ...
            for line in post:
                if line:
                    val = int(line)
            child_ids = request.env['sicpro.app.trabajadores'].sudo().browse(val).child_ids
            emp = request.env['sicpro.app.trabajadores'].sudo().browse(val)
            ...
            view = """ <div id="parent" class='o_chart_head'><a>
                ...
                <img class='o_emp_active' src='/web/image/sicpro.app.trabajadores/""" + str(val) + """/image_1024/'/></div>
                <div class='employee_name o_width'><p>""" + str(emp.name) + """</p>
                <p>""" + str(emp.ocupacion_id.name.name) + """</p></div></a></div>"""
    ...
    @http.route('/get/child/data', type='json', auth='user', csrf=False)
    def get_child_data(self, click_id):
        if click_id:
            employee = request.env['sicpro.app.trabajadores'].sudo().browse(int(click_id))
            if employee.child_ids:
                child_count = len(employee.child_ids) * 2
                value = [child_count]
