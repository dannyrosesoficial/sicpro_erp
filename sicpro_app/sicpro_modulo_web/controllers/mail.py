...
class WebMailController(MailController):
    ...
    def _redirect_to_record(cls, model, res_id, access_token=None, **kwargs):
        ...
        if uid is not None:
            ...
            if record_action['type'] == 'ir.actions.act_url' and record_action.get('target_type') != 'public':
                url_params = {'model': model, 'id': res_id, 'active_id': res_id, 'action': record_action.get('id'), }
        ...
        # se cambió el valor de redirección original para que fuera directo al login y no a la página de inicio
        # url = '/web?#%s' % url_encode(url_params)

        url = '/web/login?#%s' % url_encode(url_params)
        return request.redirect(url)
