...
class ResUsers(models.Model):
    ...
    def _login(cls, db, login, password, user_agent_env):
        ...
        try:
            ...
            _logger.info("Login fallido para db:%s login:%s para %s", db, login, ip)
            raise
        _logger.info("Login con éxito para db:%s login:%s para %s", db, login, ip)
        return user.id
    ...
    def check_for_user_simulation(self, user_id):
        if (
            user_id
            in self.env.ref("sicpro_modulo_usuario_simular.group_user_simulation").users.ids
        ):
            return True
        if (
            self.env.ref("sicpro_modulo_usuario_simular.group_user_simulation").users.ids
            and user_id
            not in self.env.ref("sicpro_modulo_usuario_simular.group_user_simulation").users.ids
