...
class MediosInformaticosImportarWizard(models.TransientModel):
    ...
    def _tipos_equipos_ids(self):
        tipos_equipos = []
        medios_importar = self.env['sicpro.app.medios.informaticos.importar'].search([('active', '=', True)])
        if medios_importar:
            for item in medios_importar:
                tipos_equipos.append(item.tipo_equipo.id)
            return tipos_equipos

    tipos_equipos = fields.Many2many('sicpro.app.medios.informaticos.tipo.equipo', 'tipos_importar_wizard_rel',
                                     'tipos_id', 'wizard_id', string="Tipos de equipo", default=_tipos_equipos_ids,
    ...
    def actualizar_datos(self):
        ...
        try:
            ...
            medios_ids = self.env['sicpro.app.medios.informaticos'].search([('active', '=', True)])
            for medio in medios_ids:
                medio_importar_ids = self.env['sicpro.app.medios.informaticos.importar'].search(
                    [('medio_informatico', '=', medio['name'])])
                if not medio_importar_ids:
                    medio.write({'active': False, 'archivado': True, 'fecha_archivado': date})
            ...
            for tipo in tipos_ids:
                medios_ids = self.env['sicpro.app.medios.informaticos'].search(
                    ['&', ('active', '=', True), ('tipo_equipo', '=', tipo.id)])
                for medio in medios_ids:
                    medio_importar_ids = self.env['sicpro.app.medios.informaticos.importar'].search(
                        [('medio_informatico', '=', medio['name'])])
                    if medio_importar_ids:
                        for medio_importar in medio_importar_ids:
                            if medio_importar:
                                ...
                                list_registros_actualizados.append(medio_importar.medio_informatico)

                medios_importar_ids = self.env['sicpro.app.medios.informaticos.importar'].search(
                    ['&', ('active', '=', True), ('tipo_equipo', '=', tipo.id)])
                for medio_importar in medios_importar_ids:
                    ...
                    if not medio:
                        self.env['sicpro.app.medios.informaticos'].create(
                            {'name': medio_importar.medio_informatico, 'equipo': medio_importar['equipo'],
                             'tipo_equipo': medio_importar['tipo_equipo'].id,
                             'denominacion': medio_importar['denominacion'], 'local': medio_importar['local'].id,
                             'emplazamiento': medio_importar['emplazamiento'].id,
                             'activo_fijo': medio_importar['activo_fijo'],
                             'no_pieza_fab': medio_importar['no_pieza_fab'],
                             'responsable': medio_importar['responsable'],
                             'trabajador_id': medio_importar.trabajador_id.id})
                        registros_creados += 1
                        list_registros_creados.append(medio_importar.medio_informatico)
                        medio_importar.sudo().unlink()

                    else:
                        medio_importar.sudo().unlink()

                text_list_registros_creados = self.lista_a_texto(list_registros_creados)
                text_list_registros_actualizados = self.lista_a_texto(list_registros_actualizados)
                text_list_registros_archivados = self.lista_a_texto(list_registros_archivados)

                self.env['sicpro.app.medios.informaticos.historial'].sudo().create(
                    {'name': "Actualización inventario", 'fecha': date, 'registros_creados': registros_creados,
                     ...
                     'list_registros_archivados': text_list_registros_archivados, 'estado': 'exito',
                     'descripcion_estado': 'Exitoso'})
            ...
            self.env['sicpro.app.medios.informaticos.historial'].sudo().create(
                {'name': "Actualización inventario", 'fecha': date, 'registros_creados': registros_creados,
                 ...
                 'list_registros_actualizados': text_list_registros_actualizados,
                 'registros_archivados': registros_archivados,
                 'list_registros_archivados': text_list_registros_archivados, 'estado': 'fallido',
                 'descripcion_estado': error.__str__()})

        action = self.env.ref('sicpro_app_medios_informaticos.medios_informaticos_importar_action').sudo().read()[0]
        return action
