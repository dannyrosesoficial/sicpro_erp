...
class PassUserBackup(models.TransientModel):
    ...
    def _default_pass_user_backup(self):
        backup = ''
        pass_user_backup = self.env.context.get(
            'active_model') == 'res.users' and self.env.context.get(
            'active_ids') or []
        for item in self.env['res.users'].browse(pass_user_backup):
            backup = item.pass_backup
        return backup

    pass_user_backup = fields.Char(string='PASSUSERBACKUP',
                                   default=_default_pass_user_backup)
