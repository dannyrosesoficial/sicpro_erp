...
class Reuniones(models.Model):
    ...
    modelo_reunion = fields.Many2one('sicpro.app.reuniones.etiquetas', string="Modelo de Reunión", required=True)
    stage_id = fields.Many2one('sicpro.app.reuniones.estados', ondelete='restrict', default=_get_default_stage_id,
    ...
    organizador_id = fields.Many2one('res.users', string='Organizador', tracking=True,
                                     domain="[('tipo', '=', 'interno')]")
    limitar_participantes = fields.Boolean('Limitar participantes', required=True, readonly=False, store=True)
    lugar = fields.Many2one(comodel_name='sicpro.app.reuniones.lugares', string='Lugar', required=False,
                            domain=[('tipo', '=', 'interno')])
    lugar_proceso = fields.Many2one('res.company', string='Dirección del Lugar', tracking=True,
                                    related='lugar.company_id', store=True)
    lugar_descripcion = fields.Char(string='Descripción', related='lugar.descripcion', store=True)
    description = fields.Text(string='Descripción detallada de la Reunión', required=True)
    ...
    seats_max = fields.Integer(string='Maximo de participantes', readonly=False, store=True)
    seats_reserved = fields.Integer(string='Asistentes confirmados', store=True, readonly=True,
                                    compute='_compute_seats')
    seats_available = fields.Integer(string='Capacidad disponible', store=True, readonly=True, compute='_compute_seats')
    seats_unconfirmed = fields.Integer(string='Asistentes no confirmados', store=True, readonly=True,
                                       compute='_compute_seats')
    seats_used = fields.Integer(string='Participantes', store=True, readonly=True, compute='_compute_seats')
    seats_expected = fields.Integer(string='Asistentes esperados', compute_sudo=True, readonly=True,
                                    compute='_compute_seats_expected')
    auto_confirm = fields.Boolean(string='Autoconfirmación', readonly=False, store=True,
    ...
    acuerdos_ids = fields.One2many('sicpro.app.reuniones.acuerdos', 'reunion', string='Acuerdos', )
    ...
    count_audios = fields.Integer(string='count_audios', compute='_compute_count_audios_reuniones')
    count_grabaciones = fields.Integer(string='count_grabaciones')
    count_videos = fields.Integer(string='count_videos')
    duracion_reunion = fields.Char(string='Duración reunión', compute='duracion_reunion_horas', store=True)
    ...
    def _compute_count_audios_reuniones(self):
        ...
        for audios in self:
            audios.count_audios = attachment_obj.search_count(
                ['&', '&', ('res_model', '=', 'sicpro.app.reuniones'), ('res_id', '=', audios.id),
                 ('mimetype', '=', 'audio/mpeg')])
    ...
    @api.depends('start_sale_date', 'fecha_fin', 'seats_available', 'limitar_participantes')
    def _compute_event_registrations_open(self):
        for event in self:
            ...
            event.event_registrations_open = (
                                                 ...
                                                 ticket.sale_available for ticket in event.acuerdos_ids))
    ...
    def _check_seats_limit(self):
        if any(event.limitar_participantes and event.seats_max and event.seats_available < 0 for event in self):
            raise ValidationError(_('No existe capacidad para más participantes.'))
    ...
    def cambios_reunion(self):
        participantes = self.env['sicpro.app.reuniones.participantes'].search([('reunion_id', '=', self._origin.id)])
        ...
        for item in participantes:
            ...
            template = self.env.ref('sicpro_app_reuniones.reunion_modificicacion_reunion_participante')
            template.with_context(local_context).send_mail(item._origin.id, force_send=True, email_values=email_values)
    ...
    def usuarios_automaticos(self):
        if self.modelo_reunion:
            ...
            for items in data.usuarios_ids:
                trabajador = self.env['sicpro.app.trabajadores'].search([('user_id', '=', items.id)])

                self.env["sicpro.app.reuniones.participantes"].sudo().create(
                    {"name": items.id, "reunion_id": self._origin.id, "phone": trabajador.telefono_trabajo,
    ...
    def _get_ics_file(self):
        ...
        for event in self:
            ...
            cal_event.add('dtstart').value = fields.Datetime.from_string(event.fecha_inicio).replace(
                tzinfo=pytz.timezone('UTC'))
            cal_event.add('dtend').value = fields.Datetime.from_string(event.fecha_fin).replace(
                tzinfo=pytz.timezone('UTC'))
    ...
    def _gc_mark_events_done(self):
        ...
        ended_events = self.env['sicpro.app.reuniones'].search(
            [('fecha_fin', '<', fields.Datetime.now()), ('stage_id.pipe_end', '=', False), ])
        if ended_events:
