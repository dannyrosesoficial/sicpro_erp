...
class ReunionesDecisiones(models.Model):
    ...
    def _busca_etiqueta_despacho(self):
        modelo_reunion = self.env['sicpro.app.reuniones.etiquetas'].search([('despacho', '=', True)]).id
        return modelo_reunion

    name = fields.Char(string='Decisión', default=lambda self: _('Decisión: '), required=True)
    description = fields.Text('Descripción', required=True)
    reunion = fields.Many2one('sicpro.app.reuniones.despachos', string="Despacho por Plan", index=True, required=False,
                              tracking=True)
    periodo_reunion = fields.Char(string='Periodo de la reunion', compute='_periodo_reunion')
    otra_reunion = fields.Char(string='Otro Motivo', required=False, tracking=True)
    ...
    responsable_ids = fields.Many2many('res.users', 'decision_responsable_user', string="Responsable", tracking=True,
                                       readonly=False, required=True, )
    participantes_ids = fields.Many2many('res.users', 'decision_participantes_user', string="Participantes",
                                         readonly=False, store=True, domain="[('tipo', '=', 'interno')]")
    cumplimiento = fields.Integer(string='Cumplimiento', store=True)
    estado = fields.Selection(
        [('borrador', 'Borrador'), ('pendiente', 'Pendiente'), ('proceso', 'En proceso'), ('revision', 'En Revisión'),
         ('liberar', 'Liberado'), ('cumplido', 'Cumplido'), ('cancelado', 'Cancelado')], string='Estados',
        default='borrador', copy=False, tracking=True, group_expand='_group_expand_estados', )
    estado_id = fields.Integer('estado_id', default=1)
    consecutivo = fields.Char(string='Consecutivo', tracking=True, copy=False, readonly=True,
                              default=lambda self: _('D-'))
    ...
    modelo_reunion = fields.Many2one('sicpro.app.reuniones.etiquetas', string="Modelo de Reunión", required=False,
    ...
    def estado_en_proceso(self):
        if self.participantes_ids:
            ...
            self.message_post(body='Decisión asignada', subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)

            for participante in seguidor:
                # envío el correo electrónico
                email_values = {'email_to': participante.partner_id.email_formatted, }
                template = self.env.ref('sicpro_app_reuniones.reunion_decision_proceso')
                template.send_mail(self.id, force_send=True, email_values=email_values)
        else:
            raise UserError("Para continuar debe agregar a los ejecutantes de la decisión.")
    ...
    def estado_en_revision(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Decisión en revisión', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        for item in self.responsable_ids:
            # envío el correo electrónico al responsable
            email_values = {'email_to': item.email_formatted, }
            template = self.env.ref('sicpro_app_reuniones.reunion_decisión_revision')
            template.send_mail(self.id, force_send=True, email_values=email_values)
    ...
    def estado_liberar(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Decisión liberada', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        ...
        for participante in secretaria:
            # envío el correo electrónico
            email_values = {'email_to': participante.partner_id.email_formatted, }
            template = self.env.ref('sicpro_app_reuniones.reunion_decision_liberar')
            template.send_mail(self.id, force_send=True, email_values=email_values)
    ...
    def estado_cumplido(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Decisión cumplida', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
        for participante in self.message_follower_ids:
            # envío el correo electrónico
            email_values = {'email_to': participante.partner_id.email_formatted, }
            template = self.env.ref('sicpro_app_reuniones.reunion_decision_cumplida')
            template.send_mail(self.id, force_send=True, email_values=email_values)
        # animación de acuerdo cumplido
        rainbow = {'effect': {'fadeout': 'slow', 'message': 'Felicidades. La decisión fue cumplida exitosamente',
                              'type': 'rainbow_man', }}
        return rainbow
        ...
        if data:
            for item in data:
                if 3 >= item.fecha_pre_fin > 0:
                    # envió la notificación a los seguidores
                    item.message_post(body='Decisión próxima a vencer', message_type='notification',
                    ...
                    for participante in participantes:
                        ...
                        template = self.env.ref('sicpro_app_reuniones.reunion_decision_proxima_vencer')
                        template.send_mail(item.id, force_send=True, email_values=email_values)
                elif item.fecha_pre_fin == 0:
                    # envió la notificación a los seguidores
                    item.message_post(body='Decisión vencida', message_type='notification',
                                      subtype_xmlid='mail.mt_comment', author_id=self.env.user.partner_id.id)
                    # Selecciono el registro de seguidores
                    for participante in item.message_follower_ids:
                        # envío el correo electrónico
                        email_values = {'email_to': participante.partner_id.email_formatted, }
                        template = self.env.ref('sicpro_app_reuniones.reunion_decision_vencida')
                        template.send_mail(item.id, force_send=True, email_values=email_values)

    @api.model_create_multi
    def create(self, vals_list):
        res = super(ReunionesDecisiones, self).create(vals_list)
        res.consecutivo = self.env['ir.sequence'].next_by_code('decisiones_consecutivo_incrementar')
        for item in res:
            ...
            # envió la notificación a los seguidores
            item.message_post(body='Decisión asignada', subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)

            # envío el correo electrónico
            for val in responsable:
                email_values = {'email_to': val.partner_id.email_formatted}
                template = self.env.ref('sicpro_app_reuniones.reunion_nueva_decision')
                template.send_mail(item.id, force_send=True, email_values=email_values, )
...
class DecisionesRechazadas(models.TransientModel):
    ...
    def action_motivo_rechazo(self):
        ...
        rechazo = self.env['sicpro.app.reuniones.decisiones'].browse(self.env.context.get('active_ids'))
        ...
        rechazo.message_post(body='Decisión rechazada', subtype_xmlid='mail.mt_comment',
                             author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
        for participante in rechazo.message_follower_ids:
            # envío el correo electrónico
            email_values = {'email_to': participante.partner_id.email_formatted, }
            template = self.env.ref('sicpro_app_reuniones.reunion_decision_rechazada')
            template.send_mail(rechazo.id, force_send=True, email_values=email_values)
        # redirecciono la salida
        action = self.env.ref('sicpro_app_reuniones.action_decisiones_view').sudo().read()[0]
        return action
...
class DecisionesCanceladas(models.TransientModel):
    ...
    def action_motivo_cancelacion(self):
        # cambio el estado interno de la solicitud
        cancelada = self.env['sicpro.app.reuniones.decisiones'].browse(self.env.context.get('active_ids'))
        for item in cancelada:
            ...
            item.cumplimiento = 0
        # envió la notificación a los seguidores
        cancelada.message_post(body='Decisión cancelada', subtype_xmlid='mail.mt_comment',
                               author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
