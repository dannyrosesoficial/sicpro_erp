...
class ReunionesAcuerdos(models.Model):
    ...
    def default_get(self, vals):
        ...
        if 'name' in vals and (not res.get('name') or res[
            'name'] == 'Titulo') and self.env.context.get(
            'default_event_name'):
            res['name'] = 'Acuerdo para %s......' % (
                    self.env.context['default_event_name'] or '')
        return res

    name = fields.Char(string='Acuerdo', default=lambda self: 'Titulo',
                       required=True)
    ...
    estado = fields.Selection(
        [('pendiente', 'Pendiente'), ('proceso', 'En proceso'),
         ('revision', 'En Revisión'), ('liberar', 'Liberado'),
         ('cumplido', 'Cumplido'), ('cancelado', 'Cancelado')],
        string='Estados', default='pendiente', copy=False, tracking=True,
        group_expand='_group_expand_estados', )
    estado_id = fields.Integer(string='estado_id', default=1)
    negociar_terminos = fields.Boolean(string='Negociar Términos',
                                       required=False)
    ...
    modelo_reunion = fields.Many2one('sicpro.app.reuniones.etiquetas',
                                     related='reunion.modelo_reunion',
    ...
    def estado_en_proceso(self):
        if self.participantes_ids:
            ...
            self.message_post(body='Acuerdo asignado',
    ...
    def estado_en_revision(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Acuerdo en revisión',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
    ...
    def estado_liberar(self):
        ...
        # if group_validar:
        #     director = group_sdirector.user_ids
        # self.message_subscribe(partner_ids=director.partner_id.ids)

        # envió la notificación a los seguidores
        self.message_post(body='Acuerdo liberado',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
    ...
    def estado_cumplido(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Acuerdo cumplido',
                          subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
        ...
        rainbow = {'effect': {'fadeout': 'slow',
                              'message': 'Felicidades. El Acuerdo fue cumplido exitosamente',
                              'type': 'rainbow_man', }}
        return rainbow
        ...
        if data:
            for item in data:
                if 3 >= item.fecha_pre_fin > 0:
                    # envió la notificación a los seguidores
                    item.message_post(body='Acuerdo próximo a vencer',
                    ...
                    # envió la notificación a los seguidores
                    item.message_post(body='Acuerdo vencido',
                                      message_type='notification',
                                      subtype_xmlid='mail.mt_comment',
                                      author_id=self.env.user.partner_id.id)
                    # Selecciono el registro de seguidores
    ...
    def create(self, vals_list):
        ...
        for item in res:
            ...
            # envió la notificación a los seguidores
            item.message_post(body='Acuerdo asignado',
                              subtype_xmlid='mail.mt_comment',
                              author_id=self.env.user.partner_id.id)
...
class AcuerdosRechazados(models.TransientModel):
    ...
    def action_motivo_rechazo(self):
        ...
        rechazo = self.env['sicpro.app.reuniones.acuerdos'].browse(
        ...
        rechazo.message_post(body='Acuerdo rechazado',
                             subtype_xmlid='mail.mt_comment',
                             author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
        ...
        action = self.env.ref(
            'sicpro_app_reuniones.action_acuerdos_view').sudo().read()[0]
        return action
...
class AcuerdosCancelados(models.TransientModel):
    ...
    def action_motivo_cancelacion(self):
        # cambio el estado interno de la solicitud
        cancelada = self.env['sicpro.app.reuniones.acuerdos'].browse(
            self.env.context.get('active_ids'))
        for item in cancelada:
            ...
            item.cumplimiento = 0
        # envió la notificación a los seguidores
        cancelada.message_post(body='Acuerdo cancelado',
                               subtype_xmlid='mail.mt_comment',
                               author_id=self.env.user.partner_id.id)
        # Selecciono el registro de seguidores
