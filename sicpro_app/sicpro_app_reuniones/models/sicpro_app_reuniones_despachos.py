...
class ReunionesDespachos(models.Model):
    ...
    participantes_ids = fields.One2many(
        'sicpro.app.reuniones.despachos.participantes', 'despacho_id',
        string='Participantes')
    agenda_ids = fields.One2many('sicpro.app.reuniones.despachos.agenda',
                                 'despacho_id',
                                 string='Agenda/Puntos a tratar', )
    comentarios_ids = fields.One2many(
        'sicpro.app.reuniones.despachos.comentarios', 'despacho_id',
        string='Comentarios de los puntos tratados', )
    decisiones_ids = fields.One2many('sicpro.app.reuniones.decisiones',
                                     'reunion', string='Decisiones', )
    estados = fields.Selection(string='Estados',
                               selection=[('sin_iniciar', 'Sin Iniciar'),
                                          ('pospuesto', 'Pospuesto'),
                                          ('cumplida', 'Cumplido'),
                                          ('cancelado', 'Cancelado'), ],
                               default='sin_iniciar', required=True,
                               group_expand='_group_expand_estados', )
    agenda_count = fields.Integer(string='Cantidad puntos',
    ...
    def _compute_agenda_count(self):
        for item in self:
            agenda_ids = self.env[
                'sicpro.app.reuniones.despachos.agenda'].sudo().search(
                [('despacho_id', '=', item.id)])
    ...
    def notificar_participantes(self):
        participantes = self.env[
            'sicpro.app.reuniones.despachos.participantes'].search(
        ...
        self.message_post(body='Nuevo Despacho',
        ...
        for item in participantes:
            ...
            template = self.env.ref(
                'sicpro_app_reuniones.despachos_nuevo_participante')
    ...
    def write(self, vals):
        ...
        for despacho in self:
            ...
            if despacho['estados'] == 'pospuesto':
                # envió la notificación a los seguidores
                despacho.message_post(body='Despacho pospuesto',
                                      subtype_xmlid='mail.mt_comment',
                                      author_id=self.env.user.partner_id.id)
                ...
                for item in participantes:
                    ...
                    email_values = {
                        'email_to': item.name.partner_id.email_formatted, }
                    template = self.env.ref(
                        'sicpro_app_reuniones.despachos_pospuesto_participante')
                    template.send_mail(despacho.id, force_send=True,
                                       email_values=email_values)

            if despacho['estados'] == 'cancelado':
                # envió la notificación a los seguidores
                despacho.message_post(body='Despacho cancelado',
                                      subtype_xmlid='mail.mt_comment',
                                      author_id=self.env.user.partner_id.id)
                ...
                for item in participantes:
                    ...
                    email_values = {
                        'email_to': item.name.partner_id.email_formatted, }
                    template = self.env.ref(
                        'sicpro_app_reuniones.despachos_cancelado_participante')
                    template.send_mail(despacho.id, force_send=True,
                                       email_values=email_values)
