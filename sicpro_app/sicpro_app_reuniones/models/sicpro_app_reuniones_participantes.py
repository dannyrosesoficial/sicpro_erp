...
class ReunionesParticipantes(models.Model):
    ...
    reunion_id = fields.Many2one('sicpro.app.reuniones', string='Reunión', required=True, readonly=True,
                                 states={'draft': [('readonly', False)]})
    name = fields.Many2one('res.users', string='Trabajador', tracking=True, domain="[('tipo', '=', 'interno')]")
    email = fields.Char(string='Correo', related='name.email', store=True, tracking=True)
    ...
    company_id = fields.Many2one('res.company', string='Proceso', related='reunion_id.company_id', store=True,
                                 readonly=True, states={'draft': [('readonly', False)]})
    fecha_invitacion = fields.Date(string='Fecha invitación', readonly=True,
        default=lambda self: fields.Date.context_today(self))
    date_closed = fields.Datetime(string='Fecha finalización', compute='_compute_date_closed', readonly=False,
                                  store=True)
    fecha_inicio_reunion = fields.Datetime(string="Inicio de reunión", related='reunion_id.fecha_inicio')
    fecha_fin_reunion = fields.Datetime(string="Fin de reunión", related='reunion_id.fecha_fin')
    state = fields.Selection(
        [('draft', 'Sin confirmar'), ('cancel', 'Cancelado'), ('open', 'Confirmado'), ('done', 'Asistido')],
        string='Estados', default='draft', readonly=True, copy=False, tracking=True)
    tipo_reunion = fields.Selection(string='Tipo Reunión', store=True, related='reunion_id.tipo_reunion')
    ...
    def _check_seats_limit(self):
        for participantes in self:
            if participantes.reunion_id.limitar_participantes and participantes.reunion_id.seats_max and participantes.reunion_id.seats_available < (
                ...
                raise ValidationError(_('No existe capacidad disponible en la reunión.'))

    # verifico que esta marcada la auto confirmación
    def _check_auto_confirmation(self):
        if any(not registration.reunion_id.auto_confirm or (
                not registration.reunion_id.seats_available and registration.reunion_id.limitar_participantes) for
    ...
    def action_confirm(self):
        ...
        self.message_post(body='Participación Confirmada', subtype_xmlid='mail.mt_comment',
    ...
    def action_cancel(self):
        ...
        # envió la notificación a los seguidores
        self.message_post(body='Participación cancelada', subtype_xmlid='mail.mt_comment',
                          author_id=self.env.user.partner_id.id)
        # envío los correo de los seguidores del registro
        for participantes in self.message_partner_ids:
            # envío el correo a los seguidores del registro
    ...
    def create(self, vals_list):
        ...
        # envió la notificación a los seguidores
        res.message_post(body='Ha sido agregado a una nueva reunión', subtype_xmlid='mail.mt_comment',
                         author_id=self.env.user.partner_id.id)
        ...
        template.with_context(local_context).send_mail(res.id, force_send=True, email_values=email_values, )
        return res
