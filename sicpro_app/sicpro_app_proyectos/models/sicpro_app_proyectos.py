...
class AppProyectos(models.Model):
    ...
    presupuesto = fields.Float(string="Presupuesto", required=True,
                               tracking=True, default="")
    fecha_emision = fields.Date(string="Emitido", required=True, )
    fecha_recibido_deoit = fields.Date(string="Recibido DEOIT",
                                       required=True, )
    ...
    def create(self, vals):
        ...
        if request.presupuesto != 0:
            self.env.user.notify_success(
                message='El Proyecto ha sido creado correctamente', title=app)
        else:
            raise UserError(_('Por favor verifique el campo Presupuesto. Valor: ' +
                              str(request.presupuesto)))
        return request

    # notificacion de "Proyecto creado" a los seguidores: esta se utiliza en
    # las acciones automatizadas del servidor
    def mensaje_proyecto_creado(self):
        display_msg = """<strong>Nuevo proyecto creado</strong>
                      ...
                      str(self.especialidad.name) + """
                          <br/>
                          <strong>Pep:</strong> """ + str(self.pep) + """
                          <br/> """
        self.message_post(body=display_msg, type="notification",
                          subtype="mt_comment")
    ...
    def write(self, vals):
        ...
        if self.active:
            ...
            self.env.user.notify_warning(
                message='El Proyecto ha sido modificado correctamente',
                title=app)
            ...
            self.env.user.notify_danger(
                message='El Proyecto ha sido archivado correctamente',
                title=app)
