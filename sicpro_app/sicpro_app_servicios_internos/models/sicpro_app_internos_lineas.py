...
class ServiciosInternosLineas(models.Model):
    ...
    apn_etecsa = fields.Boolean(string='APN ETECSA', default=False,
                                required=False, tracking=True)
    apn_nauta = fields.Boolean(string='APN NAUTA', default=False,
                               required=False, tracking=True)
    tope_voz = fields.Integer(string="Tope Voz", required=True, tracking=True)
    tope_datos = fields.Integer(string="Tope Datos", required=False,
                                tracking=True)
    ...
    trabajador_detalles = fields.Text(string="Detalles del trabajador",
                                      required=False, tracking=True)
    ...
    observaciones = fields.Text(string="Observaciones", required=False,
                                tracking=True)
    custodia_trabajador = fields.Many2one(
        comodel_name='sicpro.app.trabajadores', string='Custodia',
        required=False, tracking=True)
    custodia_plaza_id = fields.Char(string="# Plaza de Custodia",
                                    related='custodia_trabajador.plaza_id',
                                    store=True)
    custodia_company_id = fields.Many2one('res.company',
                                          string='Proceso de Custodia',
                                          related='custodia_trabajador.company_id',
                                          store=True, tracking=True)
    custodia_ocupacion_id = fields.Many2one(
        'sicpro.app.trabajadores.ocupacion', 'Puesto de trabajo Custodia',
        tracking=True, related='custodia_trabajador.ocupacion_id', store=True)
    custodia_area_id = fields.Many2one('sicpro.app.trabajadores.areas',
                                       'Departamento Custodia',
                                       related='custodia_trabajador.area_id',
                                       store=True, tracking=True)
    custodia_identification_id = fields.Char(
    ...
    fecha_recibida = fields.Date(string='Recibida', required=False,
                                 tracking=True, default=lambda
            self: fields.Date.context_today(self))
    fecha_entregada = fields.Date(string='Entregada', required=False,
                                  tracking=True)
    fecha_custodia = fields.Date(string='En custodia', required=False,
                                 tracking=True)
    fecha_cancelada = fields.Date(string='Cancelada', required=False,
                                  tracking=True)
    estado = fields.Selection(string='Estado', required=False, tracking=True,
                              selection=[('nueva', 'Nueva'),
                                         ...
                                         ('cancelada', 'Cancelada'), ],
                              default='nueva')
    apn_mixta = fields.Boolean(string='Apn_mixta',
                               compute='_onchange_apn_mixta')
    ...
    def _check_trabajador_linea_unico(self):
        ...
        if uniq:
            raise ValidationError(
                "¡La línea seleccionada, ya cuenta con un registro creado!.\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def _check_trabajador_trabajador_unico(self):
        ...
        if uniq:
            raise ValidationError(
                "¡El trabajador seleccionado, ya cuenta con un registro creado!.\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def cron_verifico_trabajador_lineas_servicios_internos(self):
        lineas = self.env['sicpro.app.servicios.internos.lineas'].search(
            [('active', '=', True)])
    ...
    def create(self, vals_list):
        records = super(ServiciosInternosLineas, self).create(vals_list)
        ...
        for res in records:
            if res.trabajador:
                ...
                template = self.env.ref(
                    'sicpro_app_servicios_internos.servicios_internos_linea_movil')
