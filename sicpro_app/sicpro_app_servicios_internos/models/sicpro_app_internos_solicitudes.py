...
class ServiciosInternosSolicitudes(models.Model):
    ...
    solicitante_correo = fields.Char(string='Correo solicitante', store=True, tracking=True,
    ...
    beneficiario_correo = fields.Char(string='Correo Beneficiario',
    ...
    beneficiario_sap = fields.Char(string='# SAP', tracking=True,
                                           related='beneficiario_trabajador.plaza_id', store=True)
    beneficiario_CI = fields.Char(string='CI', tracking=True,
    ...
    def _check_trabajador_solicitud_unico(self):
        ...
        if uniq:
            raise ValidationError("¡El trabajador seleccionado, ya tiene una solicitud en trámite!.\n\n" + MSG_SOPORTE_SICPRO)
    ...
    def action_liberar(self, ):
        if self.doc_count != 0:
            ...
            rol_revision = self.env.ref('sicpro_app_servicios_internos.grupo_app_internos_solicitudes_revision').user_ids
            ...
            action = self.sudo().env.ref('sicpro_app_servicios_internos.internos_solicitudes_anexo1_views_actions').read()[0]
            return action
        else:
            raise UserError('Debe proporcionar una documentación válida, verifíquelo ' + MSG_SOPORTE_SICPRO)
    ...
    def action_revisado(self, ):
        ...
        # busco usuarios del rol revisión de la solicitud
        rol_revision = self.env.ref('sicpro_app_servicios_internos.grupo_app_internos_solicitudes_validar').user_ids
        # agrego los seguidores al modelo
        for item in rol_revision:
            self.message_subscribe(partner_ids=item.partner_id.ids)
            # envío la notificación a los seguidores
